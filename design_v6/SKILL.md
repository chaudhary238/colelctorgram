...
- `app/` — the current BRD v1.1 mobile UI kit. The single best reference for what shipped product feels like. Each `.jsx` file is a small per-screen component you can copy or adapt; `shared.jsx` exposes the primitives (`Button`, `Tag`, `Avatar`, `VerifyBadge`, `TrustSignals`, `ProductPhoto`, `Screen`, etc.) on `window`. `v1/` holds the original prototype.

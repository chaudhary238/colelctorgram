...
function ItemDetail({ route }) {
  ...
  const { userListings, removeItem, removedSkus } = useAppState();
        ...
        <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', background: 'var(--paper)', padding: '12px 16px 30px' }}>
          {owned ? (
            ...
            <Button variant="dark" size="block" icon={<Ico d={Icons.plusCircle} size={18}/>}
              onClick={() => push({ name: 'add-listing', sku: route.sku, intelToInhand: true })}>
              I own this — Add to my collection
            </Button>
          ) : (
            <Button variant="dark" size="block" icon={<Ico d={Icons.plusCircle} size={18}/>}
              onClick={() => push({ name: 'add-listing', sku: route.sku, forcePicker: true })}>I own this — Add to collection</Button>
          )}
        </div>
      }>
      ...
      <div style={{ padding: '14px 16px 20px' }}>
        ...
        {(c.scorredReviewed || !c.intelBy) ? (
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 8, padding: '4px 10px', borderRadius: 999, background: 'var(--bone)', border: '1px solid var(--border-strong)' }}>
            <div style={{ width: 16, height: 16, borderRadius: 4, background: 'var(--bone-deep)', color: 'var(--ink-faint)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Ico d={Icons.clock} size={10} stroke={2}/>
            </div>
            <span style={{ fontSize: 11.5, color: 'var(--ink-faint)', fontWeight: 500 }}>Pending Review <span style={{ fontWeight: 400 }}>· community entry, under Scorred review</span></span>
          </div>
        )}
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 14, padding: '4px 10px', borderRadius: 999, background: 'var(--verified-teal-soft)', border: '1px solid var(--verified-teal)',
          cursor: (c.intelBy && c.intelBy !== 'you') ? 'pointer' : 'default' }}
          onClick={() => c.intelBy && c.intelBy !== 'you' && push({ name: 'profile', user: c.intelBy })}>
          <Ico d={Icons.eye} size={12} style={{ color: 'var(--verified-teal)', flexShrink: 0 }}/>
          <span style={{ fontSize: 11.5, color: 'var(--verified-teal)', fontWeight: 500 }}>
            DB Contribution by{' '}
            <span style={{ fontWeight: 700 }}>{c.intelBy ? (c.intelBy === 'you' ? '@you' : `@${c.intelBy.replace('_','.')}`) : 'Scorred'}</span>
        ...
        <div style={{ marginBottom: 16 }}>
          {!isWish && <ValueCard label={mine?.status === 'preorder' ? 'Paid / Est. value' : 'Est. value'} value={mine ? mine.value : c.est} accent="var(--ink)"/>}
        </div>

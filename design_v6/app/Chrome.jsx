...
function SealMark({ size = 26, bg = null }) {
  const img = <img src="../uploads/icon-crop.png" width={size} height={size} style={{ objectFit: 'contain', display: 'block', flexShrink: 0 }}/>;
  if (!bg) return img;
  return (
    <div style={{ width: size, height: size, borderRadius: Math.round(size * 0.22), background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <img src="../uploads/icon-crop.png" width={size * 0.72} height={size * 0.72} style={{ objectFit: 'contain', display: 'block' }}/>
    </div>
  );
}
...
function ScorredWordmark({ fontSize = 22 }) {
  ...
  return <img src="../uploads/wordmark-crop.png" width={w} height={h} style={{ objectFit: 'contain', display: 'block', flexShrink: 0 }}/>;
}
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 18px 14px', minHeight: 44 }}>
        ...
        <button onClick={() => setOverlay({ name: 'compose' })} aria-label="Create post" style={{
          ...
          background: 'var(--slate-900)', color: 'var(--paper)', border: 'none',
          ...
          return (
            <button key={t.id} onClick={() => switchTab(t.id)}
              ...
              <div style={{
                ...
                {active && isHome
                  ? <img src="../uploads/icon-crop-white.png" width={21} height={21} style={{ objectFit: 'contain', display: 'block' }}/>

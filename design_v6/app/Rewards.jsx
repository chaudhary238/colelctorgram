...
function BadgeShelf({ u, onOpen, style }) {
  ...
  return (
    <>
      ...
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: 4, ...style }}>
        {slots.map(({ b, count }, i) => (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 15px 12px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 16, letterSpacing: '-0.01em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', minWidth: 0, flex: 1 }}>{tier.name}</span>
      ...
      <div style={{ padding: '16px' }}>
        ...
        <button onClick={() => { if (!checked) { setChecked(true); flashToast('Checked in · +5 XP'); } }} disabled={checked} style={{
          ...
          <div style={{ width: 40, height: 40, borderRadius: 11, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: checked ? 'var(--bone)' : 'var(--grail-gold)', color: checked ? 'var(--ink-faint)' : 'var(--ink)' }}>
            {checked ? (
              <img className="scr-pop-in" src="../uploads/icon-crop.png" width={24} height={24} style={{ objectFit: 'contain', display: 'block' }}/>

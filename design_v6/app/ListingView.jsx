...
function ListingView({ route }) {
  ...
  const { saved, toggleSave, listingStatus, userListings } = useAppState();
  ...
  return (
    <Screen nav={false} header={null}
      ...
      <div style={{ padding: '6px 16px 20px' }}>
        {/* title + price */}
        <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 23, letterSpacing: '-0.03em', lineHeight: 1.12, margin: '0 0 4px' }}>{c.title}</h1>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12.5, color: 'var(--ink-faint)', marginBottom: 8 }}>{c.brand} · {c.scale} · {c.year} · SKU {c.sku}</div>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, marginBottom: 12, padding: '3px 9px', borderRadius: 999,
          background: 'var(--verified-teal-soft)', border: '1px solid var(--verified-teal)',
          cursor: (c.intelBy && c.intelBy !== 'you') ? 'pointer' : 'default' }}
          onClick={() => c.intelBy && c.intelBy !== 'you' && push({ name: 'profile', user: c.intelBy })}>
          <Ico d={Icons.eye} size={11} style={{ color: 'var(--verified-teal)', flexShrink: 0 }}/>
          <span style={{ fontSize: 11, color: 'var(--verified-teal)', fontWeight: 600 }}>
            DB Contribution by{' '}
            <span style={{ fontWeight: 700 }}>
              {c.intelBy ? (c.intelBy === 'you' ? '@you' : `@${c.intelBy.replace('_','.')}`) : 'Scorred'}
            </span>

          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 16 }}>
          <span style={{ fontSize: 28, fontWeight: 700, color: 'var(--stamp-red)', flexShrink: 0, whiteSpace: 'nowrap' }}><Money value={l.price}/></span>
          {l.retail > l.price && <span style={{ fontSize: 15, flexShrink: 0, whiteSpace: 'nowrap' }}><Money value={l.retail} strike/></span>}
          {l.retail > l.price && <span style={{ fontSize: 12, color: 'var(--forest)', fontWeight: 600, flexShrink: 0, whiteSpace: 'nowrap' }}>{Math.round((1 - l.price / l.retail) * 100)}% off MRP</span>}
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, overflow: 'hidden' }}>
                <span style={{ fontWeight: 600, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0 }}>{seller.name}</span>
              </div>
        ...
        <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', background: 'var(--paper)', padding: '12px 16px 30px' }}>
          {mine ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <Button variant="secondary" icon={<Ico d={Icons.edit} size={17}/>} onClick={() => { pop(); setTimeout(() => push({ name: 'add-listing', editId: l.id }), 10); }}>Edit</Button>
...
Object.assign(window, { ListingView, UserListingView, ItemListingsView, SpecRow, Stepper });

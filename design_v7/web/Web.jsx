...
function WebSidebar() {
  const { tab, switchTab, overlay, setOverlay, push } = useNav();
  ...
  return (
    <nav className="ch-sidebar" style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
        ...
        <NavItem icon={Icons.grid}     label="Database" active={onTab('database')} onClick={() => switchTab('database')}/>
        <NavItem icon={Icons.calendar} label="Events"   onClick={() => push({ name: 'events' })}/>
        ...
        <NavItem label="My Space" active={onTab('me')} onClick={() => switchTab('me')}

...
function CommunityDetail({ route }) {
  ...
  const { joined, toggleJoin, comRequested, requestJoin, cancelRequest, userCommunities, guidelinesAccepted, acceptGuidelines } = useAppState();
  ...
  return (
    <Screen header={null} footer={!locked && activeTab === 'posts' && isMember && accepted ? (
      <div style={{ background: 'var(--paper)', borderTop: '1px solid var(--slate-200)', padding: '10px 16px calc(10px + env(safe-area-inset-bottom))' }}>
        <button onClick={startCompose} style={{
          display: 'flex', alignItems: 'center', gap: 10, width: '100%', height: 46, padding: '0 16px',
          borderRadius: 14, border: '1px solid var(--slate-200)', background: 'var(--slate-50)', cursor: 'pointer',
          fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--ink-faint)', textAlign: 'left' }}>
          <Ico d={Icons.edit} size={17} stroke={1.8}/>

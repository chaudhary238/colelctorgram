...
function AuthorLine({ handle, time, community, onOpen, showFollow, reserveRight = 0 }) {
  ...
  const { followed, toggleFollow, communityRoleOverrides } = useAppState();
  ...
  const comRole = community ? roleOfWithOverride(community, handle, communityRoleOverrides) : null;
  return (
...
function PostCard({ post, showFollow = false, canModerate = false, onRemove }) {
  ...
  return (
    <div
      ...
      {/* tagged database item */}
      {item && (
        <button onClick={(e) => { e.stopPropagation(); push({ name: 'item', sku: item.sku }); }} style={{ display: 'flex', alignItems: 'center', gap: 10, width: 'calc(100% - 36px)', margin: '12px 18px 0', textAlign: 'left', padding: 8, borderRadius: 12, border: '1px solid var(--border-strong)', background: 'var(--paper-soft)', cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
          <div style={{ width: 38, height: 38, flexShrink: 0 }}><ProductPhoto tone={item.tone} ratio="1/1" rounded={9}/></div>
          ...
          <Ico d={Icons.chevR} size={15} style={{ color: 'var(--ink-faint)', flexShrink: 0 }}/>
        </button>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '12px 18px 10px' }}>
        ...
        {canModerate && (
          <ActionBtn icon={<Ico d={Icons.trash} size={19}/>} activeColor="var(--stamp-red)" active={confirmRemove} onClick={() => setConfirmRemove(v => !v)}/>
        )}
      </div>
      {confirmRemove && (
        <div style={{ margin: '0 18px 14px', padding: 12, borderRadius: 12, background: 'var(--stamp-red-soft)', border: '1px solid var(--stamp-red)' }}>
          ...
          <textarea value={removeReason} onChange={e => setRemoveReason(e.target.value.slice(0, 200))} rows={2} placeholder="e.g. Breaks community rules, off-topic, spam…"
...
function CommentThread({ post }) {
  const { push } = useNav();
  const { communityRoleOverrides } = useAppState();
  ...
  const Row = ({ c, reply, onLike, onReply, pushFn }) => {
    ...
    const comRole = community ? roleOfWithOverride(community, c.user, communityRoleOverrides) : null;
    return (
      <div style={{ display: 'flex', gap: 9 }}>
        <Avatar name={cu.name} color={cu.color} size={reply ? 26 : 30} frame={avatarFrame(cu)} framePip={avatarFramePip(cu)}/>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ background: 'var(--slate-50)', border: '1px solid var(--slate-200)', borderRadius: 14, padding: '10px 13px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              <span style={{ fontSize: 13, fontWeight: 600, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cu.name}</span>
              {comRole && <RoleBadge role={comRole}/>}
              <FeedBadgePill u={cu}/>
              <div style={{ flex: 1 }}/>
...
function CommunityCard({ com, onOpen }) {
  const { joined, toggleJoin, comRequested, requestJoin, cancelRequest, flashToast, communityRoleOverrides } = { ...useAppState(), ...useNav() };
  ...
  const youAdmin = roleCanManage(myRole);
  const pending = !communityApproved(com);
      ...
      <button onClick={onOpen} style={{ flex: 1, minWidth: 0, textAlign: 'left', background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}>
        ...
        {(pending || isPrivate || youAdmin) && (
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center', marginTop: 4 }}>
            ...
            {youAdmin && <Badge variant="teal">{myRole === 'Founder' ? 'Admin' : myRole}</Badge>}
          </div>
        )}
        <div style={{ fontSize: 11.5, color: 'var(--slate-400)', fontFamily: 'var(--font-mono)', marginTop: 4 }}>{com.members.toLocaleString('en-IN')} members</div>
      </button>
      {youAdmin
        ? <Button size="sm" variant="secondary" style={{ minWidth: 84, justifyContent: 'center' }} onClick={onOpen}>Open</Button>
        : <Button size="sm" variant={isJoined || requested ? 'secondary' : 'dark'} style={{ minWidth: 84, justifyContent: 'center' }} onClick={isJoined ? onOpen : onJoin}>{label}</Button>}
    </div>
  );
}

// ── ISO (In Search Of) card — BRD §9.x ──────────────────────
// ── ISO (In Search Of) card ─────────────────────────────────
// Structurally identical to PostCard: same shell, same author line with no type pill, same
// action row. The only ISO-specific part is the item block, which now uses the neutral card
// surface instead of a gold-bordered panel with a WANTED watermark — that read as a second,
// unrelated element sitting inside the post.
function ISOCard({ post }) {
  const { push } = useNav();
  const { hearted, saved, toggleHeart, toggleSave } = useAppState();
  const { flashToast, setOverlay } = useNav();
  const liked = hearted[post.id];
  const isSaved = saved[post.id];
  const [showComments, setShowComments] = React.useState(false);
  ...
  return (
    <div
      ...
      <div style={{ padding: '16px 18px 0' }}>
        ...
        {/* Looking for — the item being sought, in the same visual language as any other
            in-post reference block. */}
        <div style={{ marginTop: 11 }}>
          <div style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--ink-faint)', marginBottom: 4 }}>Looking for</div>
          <div style={{
            fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 17,
            letterSpacing: '-0.025em', color: 'var(--ink)', lineHeight: 1.22, marginBottom: 9,
          }}>{post.isoItem}</div>

          {refItem && (
            <button onClick={() => push({ name: 'item', sku: refItem.sku })} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', textAlign: 'left', marginBottom: 10, padding: '7px 9px', borderRadius: 10, border: '1px solid var(--border)', background: 'var(--paper-soft)', cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
              <div style={{ width: 30, height: 30, flexShrink: 0 }}><ProductPhoto tone={refItem.tone} ratio="1/1" rounded={7}/></div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{refItem.title}</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--ink-faint)' }}>{refItem.brand}</div>
              </div>
              <Ico d={Icons.chevR} size={14} style={{ color: 'var(--ink-faint)', flexShrink: 0 }}/>
            </button>
          )}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {post.isoBudget && (
              <span style={{
                ...
                padding: '4px 9px', borderRadius: 6, background: 'var(--bone)',
        ...
        {post.body && (
          <div style={{ fontSize: 15, color: 'var(--ink-soft)', lineHeight: 1.55, marginTop: 10 }}>{post.body}</div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 18px 16px' }}>
        ...
        <ActionBtn icon={<Ico d={Icons.share} size={19}/>}
          onClick={() => setOverlay({ name: 'share', label: `${userOf(post.user).name.split(' ')[0]}'s wanted post` })}/>
        <div style={{ flex: 1 }}/>
        {post.user !== 'you' && (
...
// ── Shared listing card (in feed) — seller-authored, with optional caption ──
function SharedListingCard({ post }) {
  const { push, setOverlay, flashToast } = useNav();
  const { hearted, toggleHeart, saved, toggleSave } = useAppState();
  const l = listingOf(post.listingId);
  if (!l) return null;
  const c = catOf(l.sku);
  if (!c) return null;
  ...
  return (
    <div style={{ background: 'var(--card-surface)', borderRadius: 20, margin: '0 14px 12px', padding: '16px 18px 4px', boxShadow: 'var(--card-shadow)', border: '1.5px solid var(--verified-teal)', position: 'relative' }}>
      ...
      {post.caption && <div style={{ fontSize: 14, color: 'var(--ink)', lineHeight: 1.45, marginBottom: 12 }}>{post.caption}</div>}
      <button onClick={() => push({ name: 'listing', id: post.listingId })} style={{
        display: 'flex', gap: 13, width: '100%', textAlign: 'left', border: '1px solid var(--slate-200)',
        background: 'var(--slate-50)', borderRadius: 16, padding: 12, cursor: 'pointer',
        boxShadow: 'var(--shadow-1)',
      }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 18px 16px' }}>
        <ActionBtn icon={<Ico d={Icons.heart} size={19}/>} label={(post.likes || 0) + (liked ? 1 : 0)} active={liked}
          onClick={() => toggleHeart(post.id)}/>
        <ActionBtn icon={<Ico d={Icons.comment} size={19}/>} label={commentCount} active={showComments}
          onClick={() => setShowComments(v => !v)}/>
        <ActionBtn icon={<Ico d={Icons.bookmark} size={19}/>} active={isSaved} activeColor="var(--ink)"
          onClick={() => { toggleSave(post.id); flashToast(isSaved ? 'Removed from saved' : 'Saved'); }}/>
        <ActionBtn icon={<Ico d={Icons.share} size={19}/>} onClick={() => setOverlay({ name: 'share', label: `${seller.name.split(' ')[0]}'s listing` })}/>
        <div style={{ flex: 1 }}/>
        {post.user !== 'you' && (
          <Button size="sm" variant="teal" icon={<Ico d={Icons.message} size={15}/>}
            onClick={() => push({ name: 'chat', user: post.user, intent: 'buy', listing: post.listingId })}>
            Message
          </Button>
        )}
      </div>
...
Object.assign(window, { AuthorLine, TypeRibbon, PostCard, ISOCard, CommentThread, PostImages, PollBlock, AdminCard, ListingFeedCard, SharedListingCard, MarketCard, EventCard, CommunityCard });

...
{/* Centre content */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center',
        ...
        <p className="scr-fu-2" style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--stamp-red)', margin: '0 0 2px', textAlign: 'center' }}>
...
function Onboard({ onFinish }) {
  // Onboarding collected name, city, gender and age but never saved them, so a signup's answers
  // were discarded and the profile fell back to seed values. This component sits OUTSIDE
  // AppStateProvider, so it cannot write state directly — it hands the patch up via onFinish.
  ...
  const commit = () => {
    ...
    if (city) { patch.city = city; patch.country = country; }
    if (gender) patch.gender = gender;
    if (age) patch.age = age;
    onFinish(Object.keys(patch).length ? patch : null);
  };

  const canNext = step === 1 ? chosenCats.length > 0 : true;
  const next = () => step < 2 ? setStep(step + 1) : commit();
  ...
  return (
    <div style={{ width: '100%', height: '100%', boxSizing: 'border-box', background: 'var(--paper)', display: 'flex', flexDirection: 'column', paddingTop: 52 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 20px 6px' }}>
        ...
        <button onClick={commit} style={{ width: 50, textAlign: 'right', background: 'none', border: 'none', color: 'var(--ink-faint)', fontSize: 13.5, fontWeight: 600, cursor: 'pointer' }}>Skip</button>
      </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '14px 20px 20px' }}>
        {step === 0 && <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {/* City is a picked value from a fed list — see CityPicker in shared.jsx for why. */}
            <CityPicker value={city} country={country} onChange={(c, ct) => { setCity(c); setCountry(ct); }}/>

...
function PostDetail({ route }) {
  ...
  const { hearted, toggleHeart, saved, toggleSave, posts: userPosts } = useAppState();
  const all = [...(userPosts || []), ...POSTS, ...(typeof ISO_POSTS !== 'undefined' ? ISO_POSTS : [])];
  ...
  return (
    <Screen nav={false} header={<DetailHeader title={post.type === 'iso' ? 'Wanted' : 'Post'}/>}>
      {post.type === 'iso' ? (
        <>
          <ISOCard post={post}/>
      ...
      {post.image && item && <div style={{ padding: '0 16px' }}><ProductPhoto tone={post.tone} ratio="3/2" label={item.brand}/></div>}
      {item && (
        <div style={{ padding: '12px 16px 0' }}>
          <button onClick={() => push({ name: 'item', sku: item.sku })} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--ink-faint)', fontFamily: 'var(--font-mono)' }}>view item</div>
            </div>
          </button>
        </div>
      )}

    ...
    <Screen nav={false} header={null} footer={footer}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', bottom: 14, left: 16, right: 16 }}>
            <div style={{ display: 'inline-flex', alignItems: 'baseline', gap: 6, borderRadius: 8, background: 'var(--paper)', color: 'var(--ink)', padding: '4px 10px', marginBottom: 10, boxShadow: 'var(--shadow-2)' }}>
              <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 15 }}>{ev.date}</span>
              <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--stamp-red)' }}>{ev.month}</span>
              <span style={{ fontSize: 11, color: 'var(--ink-faint)' }}>· {ev.day}</span>
            </div>
            <div style={{ color: 'var(--paper)', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 22, letterSpacing: '-0.02em', lineHeight: 1.15, textWrap: 'pretty' }}>{ev.title}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '16px' }}>
        ...
        <button onClick={() => push({ name: 'profile', user: ev.host })} style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ fontSize: 14, fontWeight: 600 }}>{isHost ? 'You' : host.name}</span></div>

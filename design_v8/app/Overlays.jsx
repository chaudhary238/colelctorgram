...
function ComposeOverlay({ community, kind: initialKind, type: initialType }) {
  ...
  const [type, setType] = React.useState(initialType || 'post'); // post | iso | poll | review (single-window switch)
  ...
  // linked database item (all post types) — BRD: tag an item from Scorred DB
  const [refSku, setRefSku]   = React.useState(null);
  const [showItem, setShowItem] = React.useState(false);
  const [itemQ, setItemQ]     = React.useState('');
  const refItem = refSku ? catOf(refSku) : null;
  const itemResults = React.useMemo(() => {
    const q = itemQ.trim().toLowerCase();
    const pool = typeof CATALOGUE !== 'undefined' ? CATALOGUE : [];
    return (q ? pool.filter(c => (c.title + ' ' + c.brand + ' ' + c.sku).toLowerCase().includes(q)) : pool).slice(0, 6);
  }, [itemQ]);
  ...
  const canPost =
    ...
    type === 'review' ? (rating > 0 && refSku && (title.trim() || body.trim())) :
  ...
  const publish = () => {
    ...
    if (type === 'review' && !refSku) { flashToast('Reviews must be tagged to a database item'); return; }
    ...
    addPost(type === 'iso' ? {
      type: 'iso', isoItem: (refItem && refItem.title) || isoItem.trim(), body: body.trim() || null, user: 'you',
      community: postTo.find(p => p !== 'feed') || null, isoBudget: isoBudget ? Number(isoBudget) : null,
      isoCond: isoConds.filter(c => c !== 'any').join(',') || 'any', image: images.length > 0, refSku,
      ...
      tags: tags.slice(), rating: type === 'review' ? rating : null, cats, cat: cats[0] || null, poll, refSku,
    });
    setOverlay(null);
    ...
    <OverlayShell title="Create a post" onBack={initialKind ? () => setOverlay(null) : () => setKind(null)} onClose={() => setOverlay(null)}
      ...
      <div style={{ padding: '14px 16px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 6 }}>
          ...
          <button onClick={() => setShowItem(v => !v)} aria-label="Tag an item from the database" style={{ display: 'flex', alignItems: 'center', gap: 5, height: 28, padding: '0 9px', borderRadius: 8, border: `1px solid ${showItem || refItem ? 'var(--ink)' : 'var(--border-strong)'}`, background: showItem || refItem ? 'var(--bone)' : 'var(--paper-soft)', color: 'var(--ink)', cursor: 'pointer', fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: 12.5, whiteSpace: 'nowrap' }}>
            <Ico d={Icons.tag} size={14}/>{refItem ? 'Item ✓' : type === 'review' ? 'Tag item *' : 'Tag item'}
          </button>
          <div style={{ flex: 1 }}/>
        ...
        {refItem && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8, padding: 9, borderRadius: 12, border: '1px solid var(--border-strong)', background: 'var(--paper-soft)' }}>
            <div style={{ width: 42, height: 42, flexShrink: 0 }}><ProductPhoto tone={refItem.tone} ratio="1/1" rounded={9}/></div>
            ...
            <button onClick={() => { setRefSku(null); setShowItem(false); }} aria-label="Remove tagged item" style={{ width: 28, height: 28, borderRadius: 9, border: '1px solid var(--border)', background: 'transparent', color: 'var(--ink-faint)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        {showItem && !refItem && (
          <div style={{ marginTop: 8, border: '1px solid var(--border-strong)', borderRadius: 12, background: 'var(--paper-soft)', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 40, padding: '0 12px', borderBottom: '1px solid var(--border)' }}>
              ...
              <input autoFocus value={itemQ} onChange={e => setItemQ(e.target.value)} placeholder="Search the Scorred database…"
            ...
            {itemResults.map((c, i) => (
              <button key={c.sku} onClick={() => { setRefSku(c.sku); setShowItem(false); setItemQ(''); if (type === 'iso' && !isoItem.trim()) setIsoItem(c.title); if (c.cat && !cats.includes(c.cat)) setCats(cs => [...cs, c.cat]); }}
                ...
                <div style={{ width: 34, height: 34, flexShrink: 0 }}><ProductPhoto tone={c.tone} ratio="1/1" rounded={8}/></div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.title}</div>
                  <div style={{ fontSize: 11.5, color: 'var(--ink-faint)' }}>{c.brand}</div>
                </div>
              </button>
            ))}
            {itemResults.length === 0 && (
              <div style={{ padding: '12px', display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'space-between' }}>
                ...
                <Button size="sm" variant="secondary" onClick={() => { setOverlay(null); push({ name: 'add-to-db' }); }}>Add a new item</Button>
              </div>
            )}
          </div>
        )}
        ...
        {showPeople && people.length > 0 && <ResGroup label="People">
          {people.slice(0, 4).map(u => (
            <ResRow key={u.handle} onClick={() => go({ name: 'profile', user: u.handle })}
              ...
              title={u.name} sub={`@${u.handle} · ${u.vouchesReceived} vouches`}/>
          ))}
        </ResGroup>}
...
function ShareSheet({ label }) {
  ...
  return (
    <div onClick={close} style={{ position: 'absolute', inset: 0, zIndex: 130, background: 'rgba(20,17,15,0.45)', display: 'flex', alignItems: 'flex-end', animation: 'fadeIn 160ms' }}>
      {/* box-sizing keeps the 18px padding INSIDE the 100% width — without it the sheet renders
         36px wider than the device frame and clips the close button and the Copy action. */}
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', boxSizing: 'border-box', maxWidth: '100%', overflowX: 'hidden', background: 'var(--paper)', borderRadius: '20px 20px 0 0', padding: '10px 18px 30px', animation: 'sheetUp 240ms cubic-bezier(0.22,1,0.36,1)' }}>
        ...
        {/* targets — the strip scrolls horizontally, so it needs its own bottom room for labels */}
        <div style={{ display: 'flex', gap: 14, overflowX: 'auto', paddingBottom: 10, margin: '0 -18px', padding: '0 18px 10px', scrollbarWidth: 'none' }}>
          {targets.map(t => (
            <button key={t.name} onClick={() => send(t.name)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7, background: 'none', border: 'none', cursor: 'pointer', flexShrink: 0, width: 60 }}>
              ...
              <span style={{ fontSize: 11.5, color: 'var(--ink-soft)', fontWeight: 500, lineHeight: 1.3, whiteSpace: 'nowrap' }}>{t.name}</span>
            </button>
          ))}
        </div>

        {/* copy link */}
        <button onClick={() => send('Copy link')} style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', boxSizing: 'border-box', textAlign: 'left', marginTop: 16, padding: '13px 14px', borderRadius: 13, border: '1px solid var(--border-strong)', background: 'var(--paper-soft)', cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
...
// ── Share to Feed — post an existing listing to the feed with a caption ──
function ShareToFeedSheet({ listingId }) {
  const { setOverlay, flashToast } = useNav();
  const { addPost } = useAppState();
  const [caption, setCaption] = React.useState('');
  const [postTo, setPostTo] = React.useState(['feed']);
  const [showPostTo, setShowPostTo] = React.useState(false);
  const togglePostTo = id => setPostTo(ps => ps.includes(id) ? (ps.length > 1 ? ps.filter(p => p !== id) : ps) : [...ps, id]);
  const l = listingOf(listingId);
  const c = catOf(l.sku);
  const close = () => setOverlay(null);
  const post = () => { addPost({ type: 'listing-share', user: 'you', listingId, caption: caption.trim() || null, community: postTo.find(p => p !== 'feed') || null }); close(); flashToast('Shared to your feed'); };
  return (
    <div onClick={close} style={{ position: 'absolute', inset: 0, zIndex: 130, background: 'rgba(20,17,15,0.45)', display: 'flex', alignItems: 'flex-end', animation: 'fadeIn 160ms' }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', boxSizing: 'border-box', maxWidth: '100%', overflowX: 'hidden', background: 'var(--paper)', borderRadius: '20px 20px 0 0', padding: '10px 18px 30px', animation: 'sheetUp 240ms cubic-bezier(0.22,1,0.36,1)' }}>
        <div style={{ width: 38, height: 4, borderRadius: 2, background: 'var(--border-strong)', margin: '0 auto 14px' }}/>
        ...
        <button onClick={() => setShowPostTo(v => !v)} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'var(--paper-soft)', border: '1px solid var(--border-strong)', borderRadius: 999, padding: '5px 10px 5px 12px', cursor: 'pointer', marginBottom: showPostTo ? 8 : 14 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)', whiteSpace: 'nowrap' }}>{postTo.includes('feed') && postTo.length === 1 ? 'Your feed' : postTo.includes('feed') ? `Feed +${postTo.length - 1}` : `${postTo.length} ${postTo.length === 1 ? 'community' : 'communities'}`}</span>
          <Ico d={Icons.chevR} size={12} stroke={2.4} style={{ transform: 'rotate(90deg)' }}/>
        </button>
        {showPostTo && (
          <div style={{ display: 'flex', flexDirection: 'column', marginBottom: 14, border: '1px solid var(--border-strong)', borderRadius: 12, overflow: 'hidden', background: 'var(--paper-soft)' }}>
            {[{ id: 'feed', name: 'Your feed' }, ...COMMUNITIES.filter(cm => cm.joined)].map((cm, i) => (
              <button key={cm.id} onClick={() => togglePostTo(cm.id)} style={{ display: 'flex', alignItems: 'center', gap: 9, height: 40, padding: '0 12px', border: 'none', borderTop: i > 0 ? '1px solid var(--border)' : 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontFamily: 'var(--font-body)', fontSize: 13.5, color: 'var(--ink)' }}>
                <span style={{ width: 16, height: 16, borderRadius: 5, border: `1.5px solid ${postTo.includes(cm.id) ? 'var(--stamp-red)' : 'var(--border-strong)'}`, background: postTo.includes(cm.id) ? 'var(--stamp-red)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {postTo.includes(cm.id) && <Ico d={Icons.check} size={11} stroke={3} style={{ color: 'var(--paper)' }}/>}
                </span>
                {cm.name}
              </button>
            ))}
        ...
        <textarea value={caption} onChange={e => setCaption(e.target.value)} placeholder="Say something about it… (optional)" rows={3} style={{ width: '100%', boxSizing: 'border-box', border: '1px solid var(--border-strong)', borderRadius: 13, padding: '12px 14px', fontFamily: 'var(--font-body)', fontSize: 14.5, color: 'var(--ink)', resize: 'none', background: 'var(--paper-soft)' }}/>
        <div style={{ display: 'flex', gap: 13, marginTop: 14, padding: 12, border: '1px solid var(--slate-200)', borderRadius: 16, background: 'var(--slate-50)' }}>
          <div style={{ width: 64, flexShrink: 0 }}><ProductPhoto tone={c.tone} ratio="1/1"/></div>
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--ink)', lineHeight: 1.25 }}>{c.title}</div>
            <div style={{ fontSize: 16, fontWeight: 700, marginTop: 4 }}><Money value={l.price}/></div>
          </div>
        </div>
        <button onClick={post} style={{ width: '100%', boxSizing: 'border-box', marginTop: 16, padding: '14px 0', borderRadius: 13, border: 'none', background: 'var(--stamp-red)', color: '#fff', fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>Post to Feed</button>
      </div>
    </div>
  );
}

Object.assign(window, { OverlayShell, ComposeOverlay, ComposeTool, SearchOverlay, NotificationsOverlay, ResGroup, ResRow, ShareSheet, ShareToFeedSheet, StarPicker });

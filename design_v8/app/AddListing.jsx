...
const FIGURE_CONDITIONS_LEGACY = [
...
function AddListingView({ route }) {
  ...
  var updateListing = appState.updateListing; var archivedListings = appState.archivedListings || {};
  var saveItemInfo = appState.saveItemInfo; var unlistListing = appState.unlistListing;

  // Archived (unlisted) listings are in the pool too — that's what makes Relist arrive prefilled.
  var allListings = [].concat(userListings || [], LISTINGS, Object.keys(archivedListings).map(function(k) { return archivedListings[k]; }));
  ...
  // itemId = editing an owned copy directly (condition/price/photos), with or without a live listing —
  // this is the one "Edit item" screen for owned items, folding what used to be two separate pages.
  var ownedItem = route.itemId ? resolveMyItems(appState).find(function(x) { return x.id === route.itemId; }) : null;
  var catalogueItem = (route.sku && !editListing) ? catOf(route.sku) : null;
  var prefill = editListing || catalogueItem || (ownedItem ? catOf(ownedItem.sku) : null);
  ...
  var photosState = React.useState(ownedItem && Array.isArray(ownedItem.photos) ? ownedItem.photos : []);
  ...
  var descState = React.useState(ownedItem && ownedItem.desc ? ownedItem.desc : (prefill && (prefill.desc || prefill.notes) ? (prefill.desc || prefill.notes) : ''));
  ...
  var condState = React.useState(editListing && editListing.condition ? editListing.condition.split(' \u00b7')[0] : (ownedItem && ownedItem.cond ? ownedItem.cond : ''));
  var cond = condState[0]; var setCond = condState[1];

  var paidState = React.useState(ownedItem && ownedItem.value ? String(ownedItem.value) : '');
  ...
  var forSaleState = React.useState(!!editListing || route.forSale === true);
  ...
  var graderState = React.useState('PSA'); var grader = graderState[0]; var setGrader = graderState[1];
  var graderOtherState = React.useState(''); var graderOther = graderOtherState[0]; var setGraderOther = graderOtherState[1];
  var gradeState = React.useState(''); var grade = gradeState[0]; var setGrade = gradeState[1];
  var certState = React.useState(''); var certNo = certState[0]; var setCertNo = certState[1];
  var isGraded = cat === 'tcg' && cond === 'Graded';
  ...
  React.useEffect(function() {
    var ids = conditionsFor(cat).map(function(x) { return x.id; });
    if (cond && ids.indexOf(cond) === -1) setCond('');
  }, [cat]);

  // Identity (category / brand / scale / title / year) is owned by the catalogue entry —
  // when we arrive from an item page or an existing listing it is shown, not re-asked.
  var identityFixed = !!(catalogueItem || editListing || ownedItem);
  var identityRef = catalogueItem || (editListing && editListing.sku ? catOf(editListing.sku) : null) || (ownedItem ? catOf(ownedItem.sku) : null);
  var identityOpen = React.useState(false);
  var idOpen = identityOpen[0]; var setIdOpen = identityOpen[1];
  ...
  var condLabel = (function() {
    if (!cond) return '';
    if (isGraded) {
      var house = grader === 'Other' ? (graderOther.trim() || 'Graded') : grader;
  ...
  var submit = function() {
    ...
    // Editing an owned copy: always save condition/price/photos back to the copy itself,
    // then create, update or drop the market listing depending on the List for sale toggle.
    if (ownedItem) {
      saveItemInfo(ownedItem.id, { cond: condLabel, value: Number(paid) || 0, photos: photos.slice(), photoCount: photos.length, desc: desc.trim() });
      var listingFields = {
        ...
        grader: isGraded ? (grader === 'Other' ? graderOther.trim() : grader) : null, grade: isGraded ? grade : null, certNo: isGraded ? certNo : null,
        acq: 'inhand', value: Number(paid) || 0,
        shipIncl: shipIncl, returns: returns, status: 'available',
        verify: 'unverified', trade: false, qty: 1, seller: 'you', mine: true,
        ships: 'India', listed: true, sku: ownedItem.sku,
      };
      if (forSale) {
        if (editListing) { updateListing(editListing.id, listingFields); }
        else { addListing(Object.assign({ id: 'u' + Date.now() }, listingFields)); }
      } else if (editListing) {
        unlistListing(editListing.id, editListing);
      }
      pop();
      flashToast(
        forSale ? (editListing ? 'Listing updated' : '+5 XP \u00b7 Listed for sale') : (editListing ? 'Unlisted \u2014 stays in your collection' : 'Saved'),
        forSale ? 'Buyers see the new terms right away' : 'Condition, price and photos updated'
      );
      return;
    }
    if (forSale) {
      ...
      // Editing an existing listing patches it in place — a new id here would leave the old
      // listing live in the market alongside the edit.
      if (route.editId && !route.relist) {
        updateListing(route.editId, saleFields);
        pop();
        flashToast('Listing updated', 'Buyers see the new terms right away');
        return;
      }
      addListing(Object.assign({ id: 'u' + Date.now() }, saleFields));
    } else {
      addListing({
        id: 'c' + Date.now(),
        title: title.trim(), brand: brand.trim(), cat: cat, tone: photos[0] ? photos[0].tone : 'ink',
  ...
  return (
    <Screen nav={false} header={<DetailHeader title={editListing || ownedItem ? 'Edit item' : catalogueItem ? 'Sell item' : 'Add an item'} subtitle={meta.label} onBack={stepInit === 'pick' ? function() { setStep('pick'); } : undefined}/>}
      footer={
        <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', background: 'var(--paper)', padding: '11px 16px 30px' }}>
          <Button variant={forSale ? 'primary' : 'dark'} size="block" onClick={submit} style={invalid ? { opacity: 0.5 } : null}
            ...
            {acq === 'intel' ? 'Contribute to DB' : ownedItem ? (forSale ? 'Save & list for sale' : 'Save') : forSale ? 'List in the Market' : 'Add to my collection'}
          </Button>
        </div>
      }>
      <div style={{ padding: '14px 16px 22px' }}>
          ...
          <div style={{ marginBottom: 4 }}>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', background: 'var(--bone)', border: '1px solid var(--border)', borderRadius: 14, padding: 12 }}>
              <div style={{ width: 54, height: 54, borderRadius: 10, overflow: 'hidden', flexShrink: 0 }}>
                <ProductPhoto tone={(identityRef && identityRef.tone) || (photos[0] && photos[0].tone) || 'ink'} ratio="1/1" rounded={10}/>
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14.5, fontWeight: 700, lineHeight: 1.25 }}>{title || 'Untitled item'}</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11.5, color: 'var(--ink-faint)', marginTop: 3 }}>
                  {[brand, usesScale && scale && scale !== 'Other' ? scale : null, year].filter(Boolean).join(' \u00b7 ')}
                </div>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '9px 2px 0' }}>
              ...
              <button onClick={function() { setIdOpen(function(v) { return !v; }); }} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', color: 'var(--stamp-red)', fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 12, whiteSpace: 'nowrap' }}>
        ...
        {acq === 'inhand' && (
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
              {conditionsFor(cat).map(function(c) {
                var on = cond === c.id;
                return (
            ...
            {(function() {
              var picked = conditionsFor(cat).filter(function(c) { return c.id === cond; })[0];
              return picked ? <div style={{ fontSize: 11.5, color: 'var(--ink-faint)', margin: '7px 2px 0' }}>{picked.hint}</div> : null;
            })()}
            {isGraded && (
              <div style={{ marginTop: 12, padding: 13, borderRadius: 13, border: '1px solid var(--border-strong)', background: 'var(--bone)' }}>
                ...
                <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 10 }}>
                  {GRADERS.map(function(g) { return <CategoryChip key={g} active={grader === g} onClick={function() { setGrader(g); }}>{g}</CategoryChip>; })}
                </div>
                {grader === 'Other' && (
                  <input value={graderOther} onChange={function(e) { setGraderOther(e.target.value); }} placeholder="Grading company"
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
                  <div style={{ width: 96, flexShrink: 0 }}>
                    ...
                    <input value={grade} onChange={function(e) { setGrade(e.target.value.replace(/[^0-9.]/g, '').slice(0, 4)); }} inputMode="decimal" placeholder="10"
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <input value={certNo} onChange={function(e) { setCertNo(e.target.value.toUpperCase()); }} placeholder="e.g. 78412095"
                ...
                <div style={{ fontSize: 11.5, color: 'var(--ink-faint)', marginTop: 9, lineHeight: 1.45 }}>Buyers can verify the slab on the grader's site with this number.</div>
              </div>
            )}

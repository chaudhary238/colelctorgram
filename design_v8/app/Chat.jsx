          ...
          return (
            <button key={c.user} onClick={() => push({ name: 'chat', user: c.user })} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {l && l.sku && <div style={{ fontSize: 11, color: 'var(--ink-faint)', margin: '2px 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>re: {catOf(l.sku).title}</div>}
...
function ChatView({ route }) {
  ...
  const handleBlock = () => {    setIsBlocked(v => !v);
    setBlockOpen(false);
    setMoreOpen(false);
    flashToast(isBlocked ? `@${u.handle} unblocked` : `@${u.handle} blocked`);
  };

  const canSubmitReport = !!reportReason && (reportReason !== 'Other' || reportNote.trim().length > 0);
                ...
                <React.Fragment>
                  ...
                  {reportReason === 'Other' && (
                    <div style={{ padding: '12px 20px 0' }}>
                      ...
                      <textarea autoFocus value={reportNote} onChange={e => setReportNote(e.target.value.slice(0, 300))} rows={3}
                      ...
                      <div style={{ display: 'flex', justifyContent: 'flex-end', fontFamily: 'var(--font-mono)', fontSize: 11, color: reportNote.length > 260 ? 'var(--stamp-red)' : 'var(--ink-ghost)', marginTop: 4 }}>{reportNote.length}/300</div>
                    </div>
                  )}
                  <div style={{ padding: '14px 20px 0' }}>
                    <Button variant="primary" style={{ width: '100%', justifyContent: 'center', opacity: canSubmitReport ? 1 : 0.45 }} onClick={canSubmitReport ? handleReport : undefined}>
                      Submit report
                    </Button>
                  </div>
                </React.Fragment>

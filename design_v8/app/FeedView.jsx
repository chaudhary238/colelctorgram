    ...
    <Screen header={<AppBar title="Scorred" wordmark leading={
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 5, background: 'var(--paper)', borderBottom: '1px solid var(--slate-200)', padding: '12px 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 4, background: 'var(--slate-100)', borderRadius: 14, padding: 4, flex: 1 }}>            {TABS.map(t => {
              const on = sort === t.id;
              return (
                <button key={t.id} onClick={() => { if (t.id === 'foryou' && on) { setCustomOpen(o => !o); } else { setSort(t.id); setCustomOpen(false); } }}
                  aria-label={t.id === 'foryou' && on ? 'Customise feed' : t.label} style={{
                  flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                  border: 'none', cursor: 'pointer', borderRadius: 10, padding: '8px 6px',
                  background: on ? 'var(--paper)' : 'transparent',
                  color: on ? (t.id === 'foryou' && customOpen ? 'var(--stamp-red)' : 'var(--ink)') : 'var(--slate-500)',
      ...
      {stream.map((x, i) => {
        if (x.t === 'post')    return x.post.type === 'listing-share' ? <SharedListingCard key={x.post.id || i} post={x.post}/> : <PostCard key={x.post.id || i} post={x.post} showFollow/>;
...
function FeedEventCard({ id }) {
  ...
  return (
    <div style={{ background: 'var(--card-surface)', borderRadius: 20, margin: '0 14px 12px', padding: '14px 18px', boxShadow: 'var(--card-shadow)', border: '1.5px solid var(--plum)' }}>

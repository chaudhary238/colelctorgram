...
function AddItemView({ route }) {
  ...
  if (picked) return <PickedForm key={picked.sku} picked={picked} popLevels={route.fromDb ? 2 : 1}/>;
      ...
      <div style={{ padding: '14px 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 9 }}>
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 9, height: 44, padding: '0 14px', borderRadius: 12, border: '1px solid var(--border-strong)', background: 'var(--paper-soft)' }}>
            ...
            <input autoFocus value={q} onChange={e => setQ(e.target.value)} placeholder="Search catalogue by name or brand…"
              style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontFamily: 'var(--font-body)', fontSize: 14.5, color: 'var(--ink)' }}/>          </div>
          ...
          {results.map(c => (
            <button key={c.sku} onClick={() => setPicked(c)} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11.5, color: 'var(--ink-faint)', fontFamily: 'var(--font-mono)', marginTop: 2 }}>{c.brand}</div>
              </div>
...
function PickedForm({ picked, popLevels = 1 }) {
  const { pop, flashToast } = useNav();
  const { addFullItem } = useAppState();
  const [est, setEst] = React.useState(String(picked.est));
  const [photoCount, setPhotoCount] = React.useState(0);
  const [status, setStatus] = React.useState('owned');
  const conds = conditionsFor(picked.cat);
  const [cond, setCond] = React.useState(conds[0].id);
  React.useEffect(() => { setCond(conditionsFor(picked.cat)[0].id); }, [picked.cat]);
  const isGraded = picked.cat === 'tcg' && cond === 'Graded';
  const [grader, setGrader] = React.useState('PSA');
  const [grade, setGrade] = React.useState('');
  const [graderOther, setGraderOther] = React.useState('');
  ...
  const submit = () => {
    const id = 'f' + Date.now();
    const base = { id, sku: picked.sku, status, verify: 'claimed', photos: photoCount };
    const item = status === 'preorder' ? {
      ...base,
      order: poOrderDate ? `Ordered ${poOrderDate}` : '',
      etaPrecision: poEtaPrec, etaDay: null,
      etaMonth: poEtaPrec === 'month' && poEtaMonth !== '' ? Number(poEtaMonth) : null,
      etaQuarter: poEtaPrec === 'quarter' && poEtaQuarter !== '' ? Number(poEtaQuarter) : null,
      etaYear: poEtaPrec !== 'tbd' ? Number(poEtaYear) : null,
      seller: poSeller.trim(), total: parseInt(poTotal, 10) || 0, deposit: parseInt(poDeposit, 10) || 0,
      value: parseInt(poTotal, 10) || 0,
    } : {
      ...base, cond, value: parseInt(est, 10) || 0,
      grader: isGraded ? (grader === 'Other' ? graderOther.trim() : grader) : null,
      grade: isGraded ? grade : null, certNo: isGraded ? certNo : null,
    };
    addFullItem(item);
    pop(popLevels);
    flashToast(`+25 XP · Added to your ${statusLabel(status)}`, status === 'owned' ? 'Counts toward your portfolio value' : 'Tracking in your pre-order calendar');
  };
      ...
      <div style={{ padding: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', background: 'var(--bone)', border: '1px solid var(--border)', borderRadius: 14, padding: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11.5, color: 'var(--ink-faint)', fontFamily: 'var(--font-mono)', marginTop: 3 }}>{[picked.brand, picked.scale !== '—' ? picked.scale : null].filter(Boolean).join(' · ')}</div>
          </div>
        </div>
        ...
        {status === 'owned' && (
          <>
            ...
            <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 9 }}>
              {conds.map(x => <CategoryChip key={x.id} active={cond === x.id} onClick={() => setCond(x.id)}>{x.label}</CategoryChip>)}
            </div>
            {(conds.find(x => x.id === cond) || {}).hint && (
              <div style={{ fontSize: 11.5, color: 'var(--ink-faint)', margin: '7px 2px 0' }}>{(conds.find(x => x.id === cond) || {}).hint}</div>
            )}
            ...
            {isGraded && (
              <div style={{ marginTop: 12, padding: 13, borderRadius: 13, border: '1px solid var(--border-strong)', background: 'var(--bone)' }}>
                ...
                <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 10 }}>
                  {GRADERS.map(g => <CategoryChip key={g} active={grader === g} onClick={() => setGrader(g)}>{g}</CategoryChip>)}
                </div>
                {grader === 'Other' && (
                ...
                <div style={{ fontSize: 11.5, color: 'var(--ink-faint)', marginTop: 9, lineHeight: 1.45 }}>The cert number lets buyers verify the slab on the grader’s site.</div>
              </div>
            )}
...
function SellView({ route }) {
  ...
  const conds = conditionsFor(c.cat);
  const [cond, setCond] = React.useState(conds[0].id);

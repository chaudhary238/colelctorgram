...
function CollectionTab({ items, u, isMe }) {
  ...
  const { dbWishlist, userListings } = st;
  const profileHandle = isMe ? 'you' : (u && u.handle) || 'you';
  const isListed = i => !!(i.sku && activeListingFor(i.sku, profileHandle, userListings, i.status, st));

  const owned = items.filter(i => i.status !== 'wishlist' && i.status !== 'intel');
  // A sold copy stays on the shelf as a record, but it is no longer value you hold.
  const ownedValue = items.filter(i => i.status === 'owned' && !i.sold).reduce((s, i) => s + i.value, 0);
  // Completeness: the value is only trustworthy — and only public — once every owned
  // item has its condition and price. See itemGaps() for the (deliberately tiny) bar.
  const needInfo = owned.filter(i => !i.sold && itemGaps(i).length);
  const allComplete = needInfo.length === 0;
  const valuePublic = allComplete;
  ...
  const filteredOwned = ownedFilter === 'all' ? owned
    : ownedFilter === 'plain' ? owned.filter(i => i.status === 'owned' && !isListed(i) && !i.sold)
    : ownedFilter === 'preorder' ? owned.filter(i => i.status === 'preorder')
    : ownedFilter === 'listed' ? owned.filter(isListed)
    : owned.filter(i => i.sold);
  ...
  return (
    <div>
      {/* portfolio value + items owned — two pills. The value is private until every item is complete:
          the owner still sees the number with a lock, a visitor sees the slot with nothing in it. */}
      <div style={{ display: 'flex', gap: 8, marginBottom: valuePublic ? 14 : 8 }}>
        <div style={{ flex: 1.3, background: 'var(--paper-soft)', border: '1px solid var(--border)', borderRadius: 999, padding: '9px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, minWidth: 0 }}>
          <span style={{ fontSize: 12, color: 'var(--ink-faint)', whiteSpace: 'nowrap' }}>{(!isMe && !valuePublic) ? 'Value not shared' : 'Portfolio Value'}</span>
          <span style={{ whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 5 }}>
            {isMe && !valuePublic && <Ico d={Icons.lock} size={12} style={{ color: 'var(--ink-faint)' }}/>}
            {(!isMe && !valuePublic)
              ? <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 15, color: 'var(--ink-faint)' }}>—</span>
              : <Money value={ownedValue} size={15}/>}
          </span>
        </div>
      ...
      {/* Collection health — one meter, not a badge per tile. Owner only. */}
      {isMe && !allComplete && (
        <button onClick={() => push({ name: 'complete-items' })} style={{
          width: '100%', textAlign: 'left', marginBottom: 14, padding: '11px 14px', borderRadius: 14, cursor: 'pointer',
          border: '1px solid var(--grail-gold)', background: 'var(--grail-gold-soft)', fontFamily: 'var(--font-body)',
          display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Ico d={Icons.lock} size={14} style={{ color: 'var(--grail-gold-deep)', flexShrink: 0 }}/>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>
                Only you can see your portfolio value
              </div>
              <div style={{ fontSize: 11.5, color: 'var(--ink-mute)', marginTop: 2 }}>
                {owned.length - needInfo.length} of {owned.length} items complete · +{needInfo.length * 20} XP to finish
              </div>
            </div>
            <span style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--grail-gold-deep)', whiteSpace: 'nowrap' }}>Finish {needInfo.length} →</span>
          </div>
          <div style={{ height: 5, borderRadius: 3, background: 'rgba(0,0,0,0.08)', overflow: 'hidden' }}>
            <div style={{ width: `${((owned.length - needInfo.length) / Math.max(owned.length, 1)) * 100}%`, height: '100%', background: 'var(--grail-gold-deep)', borderRadius: 3 }}/>
          </div>
        </button>
      )}
            ...
            {ownedFilterOpen && seg === 'owned' && (
              <>
                ...
                <div style={{
                  ...
                  boxShadow: 'var(--shadow-4)', padding: '10px', animation: 'fadeIn 140ms ease' }}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {[{ id: 'all', label: 'All' },
                      { id: 'preorder', label: `Pre-order · ${owned.filter(i => i.status === 'preorder').length}` },
                      { id: 'listed', label: `Listed · ${owned.filter(isListed).length}` },
                      { id: 'sold', label: `Sold · ${owned.filter(i => i.sold).length}` },
                      // Just owned — none of the above. Single-select alongside the rest since none
                      // of these compose (an item is exactly one of these at a time).
                      { id: 'plain', label: `Just owned · ${owned.filter(i => i.status === 'owned' && !isListed(i) && !i.sold).length}` }]
                      .map(f => {
                      const on = ownedFilter === f.id;
                      return (
                        <button key={f.id} onClick={() => { setOwnedFilter(f.id); setOwnedFilterOpen(false); }} style={{
                          ...
                          fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: on ? 700 : 500 }}>{f.label}</button>
                      );
                    })}
                  </div>
                </div>
...
function PortfolioCalendar({ items }) {
  ...
  // Month names come from data.jsx so shared.jsx (etaLabel) and the finish flow see the same list.
...
function ItemTile({ item, isMe, owner }) {
  ...
  // The tag must reflect a real listing owned by THIS profile's collector, not the shared seed flag.
  const listing = item.sku ? activeListingFor(item.sku, owner || 'you', userListings, item.status, st) : null;
  // The marker replaces the value line rather than adding a badge — the tile shows what's
  // missing in the slot where you'd look for it, and tapping goes straight to the 2-field flow.
  const gaps = isMe ? itemGaps(item) : [];
  const gapText = gapLabel(gaps);
  return (
    <div onClick={() => {
        if (!item.sku) return; // DB Contribution item — no catalogue detail page yet
        // Always the item page: it is the spine, and it carries a row through to the live
        // listing. Jumping straight to the listing used to skip the item's own details.
        push({ name: 'item', sku: item.sku, itemId: item.id, owner: owner || 'you' });
      ...
      <div style={{ position: 'relative', filter: item.sold ? 'grayscale(1)' : 'none', opacity: item.sold ? 0.62 : 1 }}>
        ...
        {item.sold ? <div style={{ position: 'absolute', top: 7, right: 7 }}><Tag kind="sold">Sold</Tag></div>
          : listing ? <div style={{ position: 'absolute', top: 7, right: 7 }}><Tag kind="sale">Listed</Tag></div>
          : item.status === 'preorder' && <div style={{ position: 'absolute', top: 7, right: 7 }}><Tag kind="po">PO</Tag></div>}
      ...
      <div style={{ padding: '8px 9px 10px' }}>
        <div style={{ fontSize: 12.5, fontWeight: 600, lineHeight: 1.25, color: item.sold ? 'var(--ink-faint)' : 'var(--ink)',
          display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', minHeight: 31 }}>{displayTitle}</div>
        {item.status !== 'intel' && item.status !== 'wishlist' && (
          item.sold ? (
            <div style={{ fontSize: 12.5, marginTop: 5, color: 'var(--ink-faint)', textDecoration: 'line-through' }}><Money value={item.value}/></div>
          ) : gapText ? (
            <button onClick={e => { e.stopPropagation(); push({ name: 'complete-items', itemId: item.id }); }} style={{

...
function CreateCommunityView({ route }) {
  ...
  const invalid = miss.name || miss.desc || miss.dup || rules.length > COMMUNITY_RULES_MAX;
  ...
  const submit = () => {
    ...
    if (forEvent) {
      ...
      pop();
      flashToast('Community submitted — pending review before it\u2019s live');
    }
  };
  const rulesOver = rules.length > COMMUNITY_RULES_MAX;
    ...
    <Screen nav={false} header={<DetailHeader title={forEvent ? 'Community for your event' : 'Create a community'} subtitle={forEvent ? 'Attendees can talk, network & post' : 'Reviewed before it goes public'}/>}
      footer={
        <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', background: 'var(--paper)', padding: '11px 16px 30px' }}>
          ...
          {!forEvent && <div style={{ textAlign: 'center', fontSize: 11.5, color: 'var(--ink-faint)', marginTop: 8 }}>Scorred reviews new communities before they're public.</div>}
        </div>
      }>
      <div style={{ padding: '4px 16px 16px' }}>
        <CCLbl hint="optional">Community photos</CCLbl>
        <div style={{ position: 'relative', marginBottom: 30 }}>
          <div style={{ position: 'relative', height: 92, borderRadius: 13, overflow: 'hidden', background: banner ? `center/cover url(${banner})` : 'var(--bone)', border: '1px solid var(--border-strong)' }}>
            ...
            <button onClick={() => bannerRef.current && bannerRef.current.click()} style={{ position: 'absolute', inset: 0, width: '100%', border: 'none', background: banner ? 'rgba(20,17,15,0.28)' : 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, color: banner ? '#fff' : 'var(--ink-mute)', fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 12.5 }}>
          ...
          <button onClick={() => photoRef.current && photoRef.current.click()} style={{ position: 'absolute', left: 14, bottom: -26, width: 60, height: 60, borderRadius: 16, border: '3px solid var(--paper)', cursor: 'pointer', background: photo ? `center/cover url(${photo})` : 'var(--ink)', color: 'var(--paper)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <textarea value={rules} onChange={e => setRules(e.target.value.slice(0, COMMUNITY_RULES_MAX))} rows={7}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11.5, color: rulesOver ? 'var(--stamp-red)' : 'var(--ink-faint)', margin: '6px 2px 0', lineHeight: 1.5 }}>
          <span>One rule per line. You can edit rules and manage members after the community goes live.</span>
          <span style={{ flexShrink: 0, marginLeft: 8 }}>{rules.length}/{COMMUNITY_RULES_MAX}</span>
        </div>

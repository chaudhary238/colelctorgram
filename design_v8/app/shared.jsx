...
// Post-type pills (ISO / Review / Showcase / Discussion) were internal taxonomy, not something
// a reader needs — a post's type is evident from its content. No pill renders.
function PostTypeTag() { return null; }
...
const STATUS_LABEL = { available: 'Available', sold: 'Sold', preorder: 'Pre-order', wishlist: 'Intel', intel: 'DB Contribution', owned: 'Owned' };
// Pre-order ETA is stored structurally (precision + parts), never as a display string, so the
// calendar can bucket it. One formatter for every view that has to show it as text.
function etaLabel(it) {
  if (!it) return '';
  const y = it.etaYear;
  const prec = it.etaPrecision || (it.etaDay != null ? 'day' : it.etaMonth != null ? 'month' : y ? 'year' : 'tbd');
  if (prec === 'tbd' || !y) return '';
  if (prec === 'year') return `${y}`;
  if (prec === 'quarter') return `Q${it.etaQuarter || 1} ${y}`;
  const m = it.etaMonth != null ? it.etaMonth : 0;
  const mo = typeof MONTH_FULL !== 'undefined' ? MONTH_FULL[m].slice(0, 3) : String(m + 1);
  return prec === 'day' && it.etaDay ? `${it.etaDay} ${mo} ${y}` : `${mo} ${y}`;
}
function hasEta(it) { return !!etaLabel(it); }

// ── Collection completeness ──────────────────────────────────
// A quick-added item counts as "complete" once the two fields that make the
// portfolio value trustworthy are present. Everything else (photos, notes,
// order dates) is optional forever — the bar is deliberately tiny.
function itemGaps(it) {
  if (!it || it.status === 'intel' || it.status === 'wishlist') return [];
  const g = [];
  if (it.status === 'preorder') {
    // An explicit "not announced yet" is an answer, not a gap — only an unanswered date counts.
    if (it.etaPrecision !== 'tbd' && !hasEta(it)) g.push('eta');
    if (it.total == null && !it.value) g.push('price');
  } else {
    if (!it.cond) g.push('cond');
    if (!it.value) g.push('price');
  }
  return g;
}
// The marker names the missing thing rather than the failure state, and it lives in
// the slot where the data would have been — so no badge layer is needed.
const GAP_LABEL = { price: 'Add price', cond: 'Add condition', eta: 'Add ETA' };
function gapLabel(gaps) {
  if (!gaps || !gaps.length) return null;
  return gaps.length > 1 ? 'Add details' : GAP_LABEL[gaps[0]];
}
function gapSentence(gaps) {
  const words = { price: 'price', cond: 'condition', eta: 'ETA' };
  const parts = (gaps || []).map(g => words[g]);
  if (!parts.length) return '';
  return 'Needs ' + (parts.length > 1 ? parts.slice(0, -1).join(', ') + ' & ' + parts[parts.length - 1] : parts[0]);
}
// My collection = seed items + quick-adds, with any later-filled info patched over the top.
// Patches and removals are keyed by item id: MY_ITEMS legitimately holds the same sku twice
// (one owned copy, one on pre-order), so sku keys would collide across distinct copies.
function resolveMyItems(st) {
  st = st || {};
  const info = st.itemInfo || {};
  const removed = st.removedSkus || {};
  const base = [...(typeof MY_ITEMS !== 'undefined' ? MY_ITEMS : []), ...(st.userItems || [])];
  return base.filter(i => !removed[i.id] && !removed[i.sku]).map(i => (info[i.id] ? { ...i, ...info[i.id] } : i));
}

// ── Cities ─────────────────────────────────────────────────────
// A fed list, not free text: typo'd and inconsistently-spelled cities ("Bangalore" /
// "Bengaluru" / "banglore") fragment every location filter, meetup search and shipping
// estimate. Each entry is [city, region, country, ...aliases] — aliases are searchable but never
// stored, so a user typing a former or short name still lands on one canonical value.
// City and country are stored SEPARATELY (never as one "City, Country" string) because event
// and listing matching compares bare city names.
const CITIES = [
  // India
...
function cityRegion(c) { return c[1] ? `${c[1]}, ${c[2]}` : c[2]; }
// How a person's location reads anywhere in the app.
function placeLabel(u) {
  if (!u) return '';
  return u.country ? `${u.city}, ${u.country}` : (u.city || '');
}

// Type-to-search city field. Matches city, region, country or alias, so "Kerala", "Japan" and
// "bombay" all surface the right rows. Reports city and country separately to the caller.
function CityPicker({ value, country, onChange, label = 'City', placeholder = 'Start typing your city…', dark = false }) {
  ...
  const matches = React.useMemo(() => {
    ...
    CITIES.forEach(c => {
      const city = c[0].toLowerCase(), region = (c[1] || '').toLowerCase(), ctry = c[2].toLowerCase();
  ...
  const pick = (c) => { onChange(c[0], c[2]); setQ(''); setOpen(false); };
  const shown = value ? (country ? `${value}, ${country}` : value) : '';

  const fieldStyle = {
    display: 'block', width: '100%', boxSizing: 'border-box', height: 48, padding: '0 40px 0 40px', borderRadius: 12,
    border: '1px solid var(--border-strong)', background: dark ? 'var(--paper)' : 'var(--paper-soft)',
  ...
  return (
    <div ref={wrap} style={{ position: 'relative' }}>
      ...
      <div style={{ position: 'relative', marginTop: 7 }}>
        <Ico d={Icons.pin} size={16} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'var(--ink-faint)', pointerEvents: 'none' }}/>
        <input
          value={open ? q : shown}
          ...
          placeholder={shown || placeholder} style={fieldStyle}/>
        {value && !open && (
          <button onClick={() => onChange('', '')} aria-label="Clear city" style={{
            position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', width: 26, height: 26, borderRadius: 8,
            border: 'none', background: 'var(--bone)', color: 'var(--ink-faint)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Ico d={Icons.close} size={14}/>
          </button>
        )}
        {open && (
          <Ico d={Icons.search} size={15} style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', color: 'var(--ink-faint)', pointerEvents: 'none' }}/>
        )}
      </div>

      {open && (
        <div style={{ position: 'absolute', left: 0, right: 0, top: '100%', marginTop: 6, zIndex: 40,
          ...
          {matches.length === 0 ? (
            ...
            <button key={c[0] + c[2]} onMouseEnter={() => setHi(i)} onClick={() => pick(c)} style={{
              ...
              <Ico d={Icons.pin} size={14} style={{ color: 'var(--ink-faint)', flexShrink: 0 }}/>
...
// Gender options — one list so signup and profile editing can never diverge.
const GENDERS = [['f', 'Female'], ['m', 'Male'], ['x', 'Prefer not to say']];
function GenderPicker({ value, onChange }) {
  return (
    <div>
      ...
      {/* Two even columns: "Prefer not to say" is much longer than the other two, so an equal
          three-across row forced it to wrap onto two lines and threw the row out of alignment. */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 7 }}>
        {GENDERS.map(([val, lbl], i) => {
          const on = value === val;
          return (
            <button key={val} onClick={() => onChange(on ? '' : val)} style={{
              ...
              fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 14.5, whiteSpace: 'nowrap' }}>{lbl}</button>
          );
        })}
      </div>
    </div>
...
// ── Disclosure ─────────────────────────────────────────────────
// One collapsible primitive for every long section (About, Specs, Rating, Discussion,
// ownership). Standardising it means each view gets the same header rhythm, the same
// chevron behaviour and the same collapsed height — the page stays short by default.
function Disclosure({ title, meta, children, defaultOpen = false, tint, icon, accent, style }) {
  ...
  return (
    <div style={{ borderTop: '1px solid var(--border)', ...style }}>
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        {icon && <Ico d={icon} size={15} style={{ color: accent || 'var(--ink-faint)', flexShrink: 0 }}/>}
        <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15, letterSpacing: '-0.015em', color: accent || 'var(--ink)' }}>{title}</span>
        {meta && <span style={{ fontSize: 12, color: 'var(--ink-faint)', fontFamily: 'var(--font-mono)' }}>{meta}</span>}
        <Ico d={Icons.chevR} size={15} style={{ marginLeft: 'auto', color: 'var(--ink-faint)', flexShrink: 0,
          transform: open ? 'rotate(90deg)' : 'none', transition: 'transform 130ms' }}/>
      </button>
...
// Long catalogue copy stays two lines until asked for — no page-length surprise from a
// verbose database entry.
function ClampText({ children, lines = 3, size = 14.5 }) {
  const [open, setOpen] = React.useState(false);
  ...
  return (
    <div>
      <div ref={ref} style={{ fontSize: size, lineHeight: 1.6, color: 'var(--ink-soft)', textWrap: 'pretty',
        ...(open ? {} : { display: '-webkit-box', WebkitLineClamp: lines, WebkitBoxOrient: 'vertical', overflow: 'hidden' }) }}>
        {children}
      </div>
      {(overflows || open) && (
        <button onClick={() => setOpen(o => !o)} style={{ marginTop: 5, padding: 0, border: 'none', background: 'none',
          cursor: 'pointer', fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: 12.5, color: 'var(--ink-mute)' }}>
...
// Single source of truth for "is this person's copy on the market right now?".
// The seed `item.listed` flag lives on shared catalogue rows, so it can't be trusted per-owner —
// always resolve against the real listings pool, scoped to whoever owns the copy.
// A pre-ordered copy is not in hand and can never be listed: without the status guard a PO copy
// of a sku you also own in hand would borrow the owned copy's listing.
// The optional `st` (whole app state) is what makes an unlisted, sold or edited listing resolve
// correctly — without it a seeded listing stays "live" after the seller pulled it.
function activeListingFor(sku, handle, userListings, status, st) {
  if (status && status !== 'owned') return null;
  const pool = [...(userListings || []), ...(typeof LISTINGS !== 'undefined' ? LISTINGS : [])];
  const isMe = !handle || handle === 'you';
  const gone = (st && st.unlisted) || {};
  const patch = (st && st.listingPatch) || {};
  const over = (st && st.listingStatus) || {};
  const found = pool.find(l => l.sku === sku && !gone[l.id] && (over[l.id] || l.status) !== 'sold'
    && (isMe ? (l.seller === 'you' || l.mine) : l.seller === handle)) || null;
  if (!found) return null;
  return { ...found, ...(patch[found.id] || {}), status: over[found.id] || found.status || 'available' };
}

// Transaction-linked trust signals row — BRD §8.2 PR-05
function TrustSignals({ u, compact = false }) {
  const items = [
    // Deals aren't tracked anywhere in the product, so the count was invented — dropped.
...
// Fractional star meter — a 4.5 average has to LOOK like 4.5, which integer stars can't show.
// Two stacked rows: outline underneath, gold clipped to the exact percentage on top.
function StarMeter({ value = 0, size = 13, gap = 1 }) {
  const pct = Math.max(0, Math.min(1, value / 5)) * 100;
  const row = (fill) => (
    <span style={{ display: 'inline-flex', gap }}>
      {[1, 2, 3, 4, 5].map(i => (
        <svg key={i} width={size} height={size} viewBox="0 0 24 24" fill={fill ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={1.6}>
  ...
  return (
    <span style={{ position: 'relative', display: 'inline-flex', color: 'var(--border-strong)', lineHeight: 0 }}>
      {row(false)}
      <span style={{ position: 'absolute', inset: 0, width: `${pct}%`, overflow: 'hidden', color: 'var(--grail-gold-deep)', lineHeight: 0 }}>
        {row(true)}
      </span>
    </span>
  );
...
Object.assign(window, {
  Ico, Icons, Avatar, Tag, PostTypeTag, VerifyBadge, TierChip, TrustSignals, Stars, statusLabel, activeListingFor,
  itemGaps, gapLabel, gapSentence, resolveMyItems, Disclosure, ClampText, etaLabel, hasEta, StarMeter, ReviewIcon,
  CITIES, cityRegion, placeLabel, CityPicker, GENDERS, GenderPicker,

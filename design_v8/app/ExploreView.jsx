...
function ExploreView() {
  ...
  const { dbWishlist, toggleDbWishlist, catalogueVersion, quickAddItem, userItems } = useAppState();
  // Only a real copy counts — a wishlisted (intel) row must stay quick-addable as owned.
  const ownedCopy = (sku) => [...(userItems || []), ...(typeof MY_ITEMS !== 'undefined' ? MY_ITEMS : [])]
    .find(i => i.sku === sku && (i.status === 'owned' || i.status === 'preorder'));
  const inCollection = (sku) => !!ownedCopy(sku);
  ...
  // The category filter is pre-seeded from your interests, which is right for browsing but wrong
  // for search: typing a title you can see in the database returned "no items match". Search spans
  // the whole catalogue unless you deliberately set filters yourself.
  const [filtersTouched, setFiltersTouched] = React.useState(false);

  const items = React.useMemo(() => {
    let l = CATALOGUE.slice();
    const searchingNow = !!q.trim();
    if (cats.length && (filtersTouched || !searchingNow)) l = l.filter(c => cats.includes(c.cat));
    ...
    return l;
  }, [q, cats, scale, sort, dbWishlist, catalogueVersion, filtersTouched]);
  ...
  const applySheet = () => { setCats(draftCats); setScale(draftScale); setSort(draftSort); setFiltersTouched(true); setSheetOpen(false); };
        ...
        {visible.map(c => (
          <div key={c.sku} style={{ background: 'var(--paper-soft)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
            <div style={{ position: 'relative' }}>
              ...
              {/* Quick add — lands the item instantly so several can be added in a row;
                  condition and price are collected later via the Finish flow. Once owned this
                  reads as a checkmark and opens the item instead of offering to add it again. */}
              <button onClick={() => {
                  if (inCollection(c.sku)) { push({ name: 'explore-item', sku: c.sku }); return; }
                  quickAddItem(c.sku);
                  flashToast('+5 XP · Added to collection', '+20 XP when you add condition & price');
                }} style={{
...
function ExploreItemDetail({ route }) {
  ...
  const { dbWishlist, toggleDbWishlist, dbRatings, rateDbItem, quickAddItem, userItems } = useAppState();
  const already = !!(typeof resolveMyItems !== 'undefined' && resolveMyItems({ userItems })
    .find(i => i.sku === route.sku && (i.status === 'owned' || i.status === 'preorder')));
  ...
  const itemComments = COMMENTS['item-' + c.sku] || [];
    ...
    <Screen nav={false} header={null}
      footer={
        <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', background: 'var(--paper)', padding: '12px 16px 30px', display: 'flex', gap: 10 }}>
          <Button variant={already ? 'secondary' : 'dark'} size="block" icon={<Ico d={already ? Icons.check : Icons.plusCircle} size={18}/>}
            onClick={() => {
              if (already) { push({ name: 'item', sku: c.sku }); return; }
              quickAddItem(c.sku);
              flashToast('+5 XP · Added to collection', '+20 XP when you add condition & price');
            }} style={{ flex: 1 }}>{already ? 'In your collection' : 'Add to collection'}</Button>
          <button onClick={() => { toggleDbWishlist(c.sku); flashToast(dbWishlist[c.sku] ? 'Removed from wishlist' : 'Added to wishlist'); }} style={{
      ...
      <div style={{ padding: '14px 16px 20px' }}>
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12.5, color: 'var(--ink-faint)', marginBottom: 10 }}>{[c.brand, c.scale !== '—' ? c.scale : null, c.year].filter(Boolean).join(' · ')}</div>

        {(c.scorredReviewed || !c.intelBy) ? (
          <div style={{ display: 'inline-flex', alignItems: 'flex-start', gap: 6, marginBottom: 8, padding: '5px 11px', borderRadius: 12, background: 'oklch(97% 0.01 30)', border: '1px solid oklch(85% 0.04 30)' }}>
            <SealMark size={16} style={{ flexShrink: 0, marginTop: 1 }}/>
            <span style={{ fontSize: 11.5, lineHeight: 1.4, color: 'var(--ink-soft)', fontWeight: 600 }}>Scorred Reviewed <span style={{ fontWeight: 400 }}>· catalogue entry verified</span></span>
          </div>
        ) : (
          <div style={{ display: 'inline-flex', alignItems: 'flex-start', gap: 6, marginBottom: 8, padding: '5px 11px', borderRadius: 12, background: 'var(--bone)', border: '1px solid var(--border-strong)' }}>
            <Ico d={Icons.clock} size={12} style={{ color: 'var(--ink-faint)', flexShrink: 0, marginTop: 2 }}/>
            <span style={{ fontSize: 11.5, lineHeight: 1.4, color: 'var(--ink-faint)', fontWeight: 500 }}>Pending Review <span style={{ fontWeight: 400 }}>· added by {c.intelBy === 'you' ? 'you' : '@' + c.intelBy}</span></span>
          </div>
        )}
        ...
        <div style={{ marginTop: 20, padding: '14px 15px', borderRadius: 14, background: 'var(--bone)', border: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10 }}>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15.5, letterSpacing: '-0.015em' }}>{mine ? 'Your rating' : 'Rate this item'}</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--ink-faint)' }}>{avg.toFixed(1)} avg · {count} ratings</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginTop: 9 }}>
            <StarRow value={mine} size={22} onRate={(n) => { rateDbItem(c.sku, n); flashToast('Thanks for rating!'); }}/>
            <span style={{ fontSize: 12.5, color: 'var(--ink-faint)' }}>{mine ? 'Tap to change' : 'Tap a star'}</span>
          </div>
        </div>
      </div>

      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10, padding: '0 18px 2px' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 16, letterSpacing: '-0.015em' }}>Discussion</div>
          <span style={{ fontSize: 12, color: 'var(--ink-faint)' }}>{itemComments.length} {itemComments.length === 1 ? 'comment' : 'comments'}</span>
        </div>
...
function ContributeItemForm() {
  ...
  const submit = () => {
    ...
    contributeToCatalogue({
      ...
      tone: 'ink', est: parseInt(est, 10) || 0, intelBy: 'you', noSku: true, desc: desc.trim(), ownersCount: 0, wishCount: 0, rating: { avg: 0, count: 0 },
    });
    setNewSku(sku);
    setSubmitted(true);
    ...
    return (
      <Screen nav={false} header={<DetailHeader title="Add to database"/>}
        footer={
          <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', background: 'var(--paper)', padding: '12px 16px 30px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            <Button variant="dark" size="block" icon={<Ico d={Icons.plusCircle} size={18}/>}
              onClick={() => push({ name: 'add-item', sku: newSku, fromDb: true })}>Add to my collection</Button>
            <Button variant="secondary" size="block" onClick={pop}>Back to database</Button>
          </div>
        }>
        <div style={{ padding: '32px 24px', textAlign: 'center' }}>
          ...
          <button onClick={() => push({ name: 'explore-item', sku: newSku })} style={{ marginTop: 14, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--stamp-red)', fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 13 }}>
  ...
  return (
    <Screen nav={false} header={<DetailHeader title="Add to database"/>}
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--ink-faint)', marginBottom: 9, lineHeight: 1.45 }}>What’s in the box, articulation, accessories, release notes — anything a collector should know.</div>
        <textarea value={desc} onChange={e => setDesc(e.target.value.slice(0, DESC_MAX))} rows={5}

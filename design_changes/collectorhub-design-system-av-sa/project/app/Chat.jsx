  ...
  return (
    <React.Fragment>
      ...
      <Screen nav={false} bodyRef={bodyRef} bg="var(--bone)"
        header={<DetailHeader title={u.name} subtitle={`${u.vouchesReceived} vouches · ${u.deals} deals · ${u.response}`}
          ...
          {dealState === 'confirmed' && (
            <div style={{ marginBottom: 10, background: 'var(--forest-soft)', border: '1px solid var(--forest)', borderRadius: 12, padding: '10px 12px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8 }}>
                <span style={{ fontSize: 12.5, color: 'var(--ink-mute)', flex: 1 }}>This deal is recorded on both profiles.</span>
                <Button size="sm" variant="teal" icon={<Ico d={Icons.shield} size={14}/>} onClick={() => push({ name: 'vouch', user: handle, mode: 'give' })}>Leave a vouch</Button>
              </div>
            </div>
          )}
      ...
      {l && (() => {
        ...
        <button onClick={() => push({ name: 'listing', id: l.id })} style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 13 }}><Money value={l.price} currency={l.sym || '₹'}/> · <span style={{ color: 'var(--ink-faint)' }}>{(l.condition || '').split('·')[0]}</span></div>
          </div>

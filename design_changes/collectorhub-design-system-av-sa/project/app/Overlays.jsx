...
function OverlayShell({ children, onClose, onBack, title, trailing }) {
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 120, background: 'var(--paper)', display: 'flex', flexDirection: 'column', animation: 'sheetUp 240ms cubic-bezier(0.22,1,0.36,1)' }}>
      <div style={{ flexShrink: 0, paddingTop: window.CH_WEB ? 14 : 52, borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px 12px', minHeight: 40 }}>
          <button onClick={onBack || onClose} aria-label={onBack ? 'Back' : 'Close'} style={{ width: 38, height: 38, borderRadius: 11, border: '1px solid var(--border)', background: 'transparent', color: 'var(--ink)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
...
function ComposeOverlay({ community }) {
  ...
  const publish = () => {
    ...
    addPost(type === 'iso' ? {
      ...
      community: com || null, isoBudget: isoBudget ? Number(isoBudget) : null,
      isoCond: isoCond, image: images.length > 0,
      images: images.length > 0 ? images.map(i => i.url || i) : undefined,
      tone: (images[0] && images[0].tone) || 'gold', tags: tags.slice(), cat: null,
    } : {
      type, title: title.trim() || null, body: body.trim(), user: 'you', community: com || null,
      images: images.map(i => i.url || i), imageCount: images.length, image: images.length > 0,
      tone: (images[0] && images[0].tone) || 'ink',
      tags: tags.slice(), rating: type === 'review' ? rating : null, cat: null, poll,
    });
    setOverlay(null);
  ...
  if (!kind) {
    const OPTIONS = [
      { id: 'post',    label: 'Create a Post',    desc: 'Showcase, ask, review or poll the community.', c: 'var(--plum)',      icon: Icons.edit },
      { id: 'listing', label: 'Add an item', desc: 'Add to your shelf — and list it for sale if you like.',     c: 'var(--stamp-red)', icon: Icons.tag },
    ];
    return (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {OPTIONS.map(o => (
              <button key={o.id} onClick={() => {
                ...
                background: 'var(--paper-soft)', border: '1px solid var(--border)', borderRadius: 16, padding: 16 }}>
                <div style={{ width: 50, height: 50, borderRadius: 13, background: o.c, color: 'var(--paper)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Ico d={o.icon} size={23}/>
                </div>
                <div style={{ flex: 1 }}>
  ...
  return (
    <OverlayShell title="Create a post" onBack={() => setKind(null)} onClose={() => setOverlay(null)}
      ...
      <div style={{ padding: '14px 16px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16, alignItems: 'center' }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 600 }}>You<span style={{ color: 'var(--ink-faint)', fontWeight: 400 }}> · {com ? 'posting to a community' : 'posting to your feed'}</span></div>
        </div>
        ...
        {/* body */}
        <textarea value={body} onChange={e => setBody(e.target.value.slice(0, BODY_MAX))} rows={type === 'poll' ? 2 : 4} placeholder={placeholder}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button onClick={() => setShowEmoji(v => !v)} aria-label="Add emoji" style={{ display: 'flex', alignItems: 'center', gap: 6, height: 32, padding: '0 11px', borderRadius: 9, border: `1px solid ${showEmoji ? 'var(--ink)' : 'var(--border-strong)'}`, background: showEmoji ? 'var(--bone)' : 'var(--paper-soft)', color: 'var(--ink)', cursor: 'pointer', fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: 13 }}>
            <span style={{ fontSize: 15 }}>😊</span>Emoji
          </button>
          <div style={{ flex: 1 }}/>
        ...
        {type === 'iso' && (
          <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 5, marginTop: 8, flexWrap: 'wrap' }}>
                  {['Any', 'Sealed', 'MIB', 'BIB', 'Loose'].map(c => (
                    <button key={c} onClick={() => setIsoCond(c.toLowerCase())}
                      style={{ padding: '6px 10px', borderRadius: 7, border: `1px solid ${isoCond === c.toLowerCase() ? 'var(--stamp-red)' : 'var(--border-strong)'}`, background: isoCond === c.toLowerCase() ? 'var(--stamp-red)' : 'var(--paper-soft)', color: isoCond === c.toLowerCase() ? 'var(--paper)' : 'var(--ink)', fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap' }}>{c}</button>
                  ))}
                </div>
              </div>
            </div>
            <div>
              ...
              <textarea value={body} onChange={e => setBody(e.target.value.slice(0, BODY_MAX))} placeholder="Variant, colourway, packaging notes..."
                rows={3} style={{ width: '100%', boxSizing: 'border-box', marginTop: 8, padding: '10px 13px', borderRadius: 11, border: '1px solid var(--border-strong)', background: 'var(--paper-soft)', fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--ink)', outline: 'none', resize: 'none' }}/>
            </div>
          </div>
        )}
        {type === 'poll' && (
        ...
        {type !== 'poll' && (
          <div style={{ marginTop: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 9, overflowX: 'auto', paddingBottom: 2 }}>
              {images.map((img, i) => (
                <div key={i} style={{ position: 'relative', width: 84, height: 84, flexShrink: 0, borderRadius: 12, overflow: 'visible' }}>
                  {img.url
                    ? <img src={img.url} style={{ width: 84, height: 84, objectFit: 'cover', borderRadius: 12, display: 'block' }}/>
                    : <ProductPhoto tone={img.tone || img} ratio="1/1" rounded={12}/>}
                  <button onClick={() => rmImg(i)} aria-label="Remove" style={{ position: 'absolute', top: -6, right: -6, width: 22, height: 22, borderRadius: '50%', cursor: 'pointer', background: 'var(--ink)', color: 'var(--paper)', border: '2px solid var(--paper)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Ico d={Icons.close} size={11} stroke={3}/></button>
                </div>
              ))}
              {images.length < 6 && (
                <button onClick={addImg} style={{ width: 84, height: 84, flexShrink: 0, borderRadius: 12, cursor: 'pointer', border: '1px dashed var(--border-strong)', background: 'var(--paper-soft)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, color: 'var(--ink-mute)' }}>
                  <Ico d={Icons.camera} size={20}/><span style={{ fontSize: 10.5, fontWeight: 600 }}>Photo</span>
                </button>
              )}
            </div>
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10, height: 44, padding: '0 13px', borderRadius: 11, border: '1px solid var(--border-strong)', background: 'var(--paper-soft)' }}>
          ...
          <input value={tagInput} onChange={e => setTagInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); addTag(); } }}
            placeholder="Add a tag — e.g. HotToys" style={{ flex: 1, minWidth: 0, border: 'none', background: 'transparent', outline: 'none', fontFamily: 'var(--font-body)', fontSize: 14.5, color: 'var(--ink)' }}/>
        ...
        {tags.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 10 }}>
            {tags.map(t => (
              <span key={t} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 7px 5px 11px', borderRadius: 999, background: 'var(--ink)', color: 'var(--paper)', fontSize: 12.5, fontWeight: 600 }}>
                #{t}
                <button onClick={() => rmTag(t)} aria-label="Remove tag" style={{ width: 18, height: 18, borderRadius: '50%', border: 'none', background: 'rgba(244,239,230,0.2)', color: 'var(--paper)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Ico d={Icons.close} size={10} stroke={3}/></button>
              </span>
            ))}
          </div>
        )}
...
function SearchOverlay() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 120, background: 'var(--paper)', display: 'flex', flexDirection: 'column', animation: 'fadeIn 160ms' }}>
      <div style={{ flexShrink: 0, borderBottom: '1px solid var(--border)', padding: window.CH_WEB ? '16px 14px 12px' : '52px 14px 12px' }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', background: 'var(--paper)', borderRadius: '20px 20px 0 0', padding: '10px 18px 30px', animation: 'sheetUp 240ms cubic-bezier(0.22,1,0.36,1)' }}>
        ...
        <button onClick={() => send('Copy link')} style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', textAlign: 'left', marginTop: 18, padding: '13px 14px', borderRadius: 13, border: '1px solid var(--border-strong)', background: 'var(--paper-soft)', cursor: 'pointer' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--ink-faint)', fontFamily: 'var(--font-mono)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>collectorhub.app/s/9f2a1</div>
          </div>

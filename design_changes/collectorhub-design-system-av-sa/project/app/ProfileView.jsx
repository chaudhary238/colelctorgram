...
function ProfileView({ route }) {
  ...
  const TradesTab = window.TradesTab || (() => <EmptyNote>No trades yet.</EmptyNote>);
    ...
    <Screen header={header} nav={isMe}>
      ...
      <div style={{ padding: '18px 16px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 18, alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative' }}>
              {isMe ? (
                <button onClick={() => push({ name: 'edit-avatar' })} aria-label="Change profile photo"
                  ...
                  <Avatar name={u.name} color={u.color} photo={u.photo} size={76}/>
              ...
              {!isMe && topBadge(u) && (
                <span style={{ position: 'absolute', right: -4, bottom: -4, borderRadius: '50%', boxShadow: '0 0 0 2.5px var(--paper)' }}>
                  <SeasonBadge b={topBadge(u)} size={26}/>
                </span>
              )}
            </div>
          ...
          {/* 3 key stats — fills the right space with meaning */}
          <div style={{ flex: 1, display: 'flex', justifyContent: 'space-around', alignItems: 'flex-start' }}>
            {[{
              n: u.deals, label: 'Deals', onClick: null,
            }, {
              n: u.followers, label: 'Followers', onClick: () => push({ name: 'follows', user: u.handle, mode: 'followers' }),
            }, {
              n: u.vouchesReceived, label: 'Vouches', onClick: () => push({ name: 'vouches', user: u.handle, mode: 'received' }),
            }].map(({ n, label, onClick }) => (
              <button key={label} onClick={onClick} disabled={!onClick} style={{
                background: 'none', border: 'none', padding: '4px 8px', cursor: onClick ? 'pointer' : 'default',
                display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
              }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 22, color: 'var(--ink)', lineHeight: 1, fontFeatureSettings: '"tnum" 1' }}>{compactNum(n)}</span>
                <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--ink-faint)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>{label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginTop: 14 }}>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 20, letterSpacing: '-0.02em', color: 'var(--ink)' }}>{u.name}</div>
          ...
          <BadgeShelf u={u} style={{ marginTop: 10 }} onOpen={() => push({ name: 'badges', user: handle })}/>
        </div>

        {/* follow + vouch stats — BRD §8.2 / §9.11 */}
        <div style={{ display: 'flex', alignItems: 'stretch', background: 'var(--paper-soft)', border: '1px solid var(--border)', borderRadius: 14, marginTop: 14, overflow: 'hidden' }}>
          <StatTile icon={Icons.users} n={u.followers} l="Followers"
            onClick={() => push({ name: 'follows', user: u.handle, mode: 'followers' })}/>
          <StatDivider/>
          <StatTile icon={Icons.userPlus} n={u.following} l="Following"
            onClick={() => push({ name: 'follows', user: u.handle, mode: 'following' })}/>
          <StatDivider/>
          <StatTile icon={Icons.shield} accent="var(--verified-teal)" n={u.vouchesReceived + (isMe ? 0 : (vouched[handle] ? 1 : 0))} l="Vouches In"
            onClick={() => push({ name: 'vouches', user: u.handle, mode: 'received' })}/>
          <StatDivider/>
          <StatTile icon={Icons.shield} accent="var(--verified-teal)" n={u.vouchesGiven + (isMe ? Object.keys(vouched).length : 0)} l="Vouches Out"
            onClick={() => push({ name: 'vouches', user: u.handle, mode: 'given' })}/>
        </div>
        ...
        {isMe ? (
          <div style={{ display: 'flex', gap: 9, marginTop: 14 }}>
            <Button variant="dark" style={{ flex: 1, justifyContent: 'center' }} icon={<Ico d={Icons.edit} size={16}/>}
              onClick={() => push({ name: 'edit-profile' })}>Edit profile</Button>
            <Button variant="secondary" icon={<Ico d={Icons.plusCircle} size={17}/>}
              onClick={() => push({ name: 'add-listing' })}>Add item</Button>
            <IconButton icon={<Ico d={Icons.settings} size={18}/>} onClick={() => push({ name: 'settings' })}/>
          </div>
        ) : (
      ...
      <div style={{ padding: '14px 16px 28px' }}>
        ...
        {tab === 'saved' && isMe && (() => {
          ...
          return savedPosts.length
            ? <div style={{ margin: '0 -16px' }}>{savedPosts.map(p => <PostCard key={p.id} post={p}/>)}</div>
...
const VOUCH_RELATIONS = [
  'Traded on CollectorHub', 'Traded off-app', 'Met in person',
...
function VouchView({ route }) {
  ...
  const REL = [
    { id: 'app', label: 'Traded on CollectorHub', icon: Icons.swap },

      ...
      <div style={{ padding: '16px' }}>
        ...
        <button onClick={() => { setHasImage(true); flashToast('In-app camera capture — proves you own it (no gallery upload)'); }} style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--ink-faint)' }}>{hasImage ? 'Upgrades to Shown / Verified ownership' : 'In-app camera only · at least one required'}</div>
          </div>
        </button>
        ...
        {status === 'preorder' && (
          <div style={{ background: 'var(--paper-soft)', border: '1px solid var(--border)', borderRadius: 13, padding: 14, marginTop: 14 }}>
            <Field label="Order date" value="Today"/>
            <Field label="Expected ship" value="Set a date" placeholder/>
          </div>
        )}
      </div>
    </Screen>
...
function SellView({ route }) {
  ...
  return (
    <Screen nav={false} header={<DetailHeader title="Sell / Trade"/>}
      footer={
        <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', background: 'var(--paper)', padding: '12px 16px 30px' }}>
          <Button variant="primary" size="block" disabled={!verified}
      ...
      <div style={{ padding: '16px' }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 18 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ marginTop: 6 }}><VerifyBadge tier={item ? item.verify : 'claimed'}/></div>
          </div>
        </div>

        {!verified && (
          <div style={{ background: 'var(--stamp-red-soft)', border: '1px solid var(--stamp-red)', borderRadius: 12, padding: '11px 13px', marginBottom: 18, display: 'flex', gap: 9, alignItems: 'flex-start' }}>
            <Ico d={Icons.shield} size={17} style={{ color: 'var(--stamp-red-deep)', flexShrink: 0, marginTop: 1 }}/>
            <div style={{ fontSize: 12.5, color: 'var(--ink-soft)', lineHeight: 1.45 }}>This item is <b>{item ? item.verify : 'claimed'}</b>. Listings must be <b>Verified</b> — capture an in-app photo + challenge shot first.</div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 52, padding: '0 14px', borderRadius: 12, border: '1px solid var(--border-strong)', background: 'var(--paper-soft)', margin: '10px 0 18px' }}>
          ...
          <input value={price} onChange={e => setPrice(e.target.value.replace(/[^0-9]/g, ''))} inputMode="numeric"
            style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: 22, color: 'var(--ink)' }}/>
          <span style={{ fontSize: 12, color: 'var(--ink-faint)' }}>MRP ~₹{c.est.toLocaleString('en-IN')}</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: 'var(--paper-soft)', border: '1px solid var(--border)', borderRadius: 13 }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 600 }}>Open to trades</div>
            <div style={{ fontSize: 12, color: 'var(--ink-faint)' }}>Buyers can propose an item swap</div>
          </div>
          <Toggle on={trade} onClick={() => setTrade(v => !v)}/>
        </div>
      </div>
    </Screen>
  );

...
function ListingView({ route }) {
  ...
  const { saved, toggleSave, listingStatus } = useAppState();
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 3 }}>
          <DetailHeader transparent
            trailing={<>
              ...
              <IconButton icon={<Ico d={Icons.share} size={17}/>} onClick={() => flashToast('Link copied')}/>
            </>}/>
        </div>
        <ProductPhoto tone={c.tone} ratio="1/1" rounded={0} label={`${l.sku} · ${photo + 1} of ${l.photos}`}>
          {status !== 'available' && (
            <div style={{ position: 'absolute', top: 60, left: 16 }}><Tag kind={sold ? 'sold' : 'reserved'} style={{ fontSize: 12, padding: '5px 10px' }}>{status}</Tag></div>
          )}
        </ProductPhoto>
      </div>
        {/* photo dots */}
      ...
      <div style={{ padding: '6px 16px 20px' }}>
        ...
        <button onClick={() => push({ name: 'profile', user: l.seller })} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                <span style={{ fontWeight: 600, fontSize: 15 }}>{seller.name}</span><TierChip tier={seller.tier}/>
              </div>
              <div style={{ fontSize: 12.5, color: 'var(--ink-faint)', marginTop: 1 }}>@{seller.handle} · {seller.city}</div>
            </div>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', background: 'var(--grail-gold-soft)', border: '1px solid var(--grail-gold)', borderRadius: 13, padding: '12px 14px', marginTop: 16 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.5 }}>
            <b>Safe trading:</b> deals complete off-platform in Phase 1. CollectorHub doesn’t hold payments. Always check trust signals, ask for an in-hand video, and never pay before you’ve verified the seller.
          </div>
        </div>
      </div>
    </Screen>
...
Object.assign(window, { ListingView, SpecRow, Stepper });

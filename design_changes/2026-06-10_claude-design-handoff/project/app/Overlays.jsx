...
function ComposeOverlay({ community }) {
  ...
  if (!kind) {
    const OPTIONS = [
      ...
      { id: 'listing', label: 'Create a Listing', desc: 'List an item for sale on the marketplace.',     c: 'var(--stamp-red)', icon: Icons.tag },
    ];
    return (
  ...
  // ── Stage 2: compose ──
  return (
    <OverlayShell title="Create a Post" onBack={() => setType(null)} onClose={() => setOverlay(null)}
...
function NotificationsOverlay() {
  ...
  const unreadIn = (cat) => NOTIFICATIONS.filter(n =>
  ...
  const shown = activeCat
    ? NOTIFICATIONS.filter(n => activeCat.kinds ? activeCat.kinds.includes(n.kind) : !CLAIMED_KINDS.includes(n.kind))
    : NOTIFICATIONS;

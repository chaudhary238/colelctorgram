          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {OPTIONS.map(o => (
              <button key={o.id} onClick={() => {
                ...
                background: 'var(--card-surface)', border: '1px solid var(--slate-200)', borderRadius: 16, padding: 16, boxShadow: 'var(--card-shadow)' }}>
    ...
    <OverlayShell title="Create a post" onBack={() => setKind(null)} onClose={() => setOverlay(null)}
      ...
      <div style={{ padding: '14px 16px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16, alignItems: 'center' }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 600 }}>You<span style={{ color: 'var(--ink-faint)', fontWeight: 400 }}> · {postTo.includes('feed') && postTo.length === 1 ? 'your feed' : postTo.includes('feed') ? `feed + ${postTo.length - 1} more` : `${postTo.length} ${postTo.length === 1 ? 'community' : 'communities'}`}</span></div>
        </div>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 5, marginTop: 8, flexWrap: 'wrap' }}>
                  {['Any', 'Sealed', 'MIB', 'BIB', 'Loose'].map(c => (
                    <button key={c} onClick={() => toggleIsoCond(c.toLowerCase())}
                      style={{ padding: '6px 10px', borderRadius: 7, border: `1px solid ${isoConds.includes(c.toLowerCase()) ? 'var(--stamp-red)' : 'var(--border-strong)'}`, background: isoConds.includes(c.toLowerCase()) ? 'var(--stamp-red)' : 'var(--paper-soft)', color: isoConds.includes(c.toLowerCase()) ? 'var(--paper)' : 'var(--ink)', fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap' }}>{c}</button>
                  ))}
                </div>
              </div>
            </div>
        ...
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 10 }}>
          <CategoryChip active={postTo.includes('feed')} onClick={() => togglePostTo('feed')}>Your feed</CategoryChip>
          {COMMUNITIES.filter(c => c.joined).map(c => <CategoryChip key={c.id} active={postTo.includes(c.id)} onClick={() => togglePostTo(c.id)}>{c.name}</CategoryChip>)}
        </div>
      </div>
    </OverlayShell>
  );

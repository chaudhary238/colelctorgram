...
const CAT_BRANDS = {
  ...
  tcg: [
    'The Pokémon Company', 'Bandai (One Piece TCG)', 'Wizards of the Coast (MTG)',
    'Konami (Yu-Gi-Oh!)', 'Bandai (Digimon TCG)', 'Bandai (Dragon Ball Super TCG)',
    'Bushiroad (Weiss Schwarz)', 'Bushiroad (Cardfight!! Vanguard)',
  ],
};
...
function AddListingView({ route }) {
  ...
  const { addListing, userListings } = useAppState();
  ...
  const changeCat = (id) => { setCat(id); setScale(''); setScaleOther(''); setSize(''); setTcgLang(''); setTcgFormat(''); setTcgGraded(false); setTcgGrade(''); };
  ...
  const submit = () => {
    ...
    if (forSale) {
      addListing({
        ...
        trade: false, shipIncl, returns, status: 'available', verify: 'claimed',
  ...
  return (
    <Screen nav={false} header={<DetailHeader title={editListing ? 'Edit listing' : catalogueItem ? 'Sell item' : 'Add an item'} subtitle={meta.label}/>}
      footer={
      ...
      <div style={{ padding: '14px 16px 22px' }}>
        ...
        {usesScale ? (
          ...
          <>
            <div style={{ marginTop: 22 }}><Lbl hint="optional">Language / Print</Lbl></div>
            <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
              {['EN', 'JP', 'KR', 'TW', 'Other'].map(l => (
            ...
            <div style={{ marginTop: 18 }}><Lbl hint="optional">Product type</Lbl></div>
            <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
              {['Single Card', 'Booster Pack', 'Booster Box', 'Elite Trainer Box', 'Collection Box', 'Case'].map(f => (
                <CategoryChip key={f} active={tcgFormat === f} onClick={() => setTcgFormat(tcgFormat === f ? '' : f)}>{f}</CategoryChip>
              ))}
            </div>

            <div style={{ marginTop: 18, padding: '0 1px' }}>
              <ToggleRow title="Graded card" sub="PSA / BGS / CGC professional grade" on={tcgGraded} onToggle={() => setTcgGraded(v => !v)} last/>
            </div>
            {tcgGraded && (
              <div style={{ marginTop: 10, display: 'flex', gap: 9, alignItems: 'flex-end' }}>
                <div style={{ flex: 1 }}>
                  <Lbl>Grading company</Lbl>
                ...
                <div style={{ width: 88 }}>
                  ...
                  <input value={tcgGrade} onChange={e => setTcgGrade(e.target.value.replace(/[^0-9.]/g, '').slice(0, 4))} inputMode="decimal" placeholder="e.g. 9"
                    style={{ ...fieldStyle(false), height: 40, fontSize: 16, fontFamily: 'var(--font-mono)', fontWeight: 700, textAlign: 'center' }}/>
                </div>
              </div>
            )}
          </>
        ...
        {acq === 'inhand' ? (
          ...
          <div style={{ marginTop: 16, background: 'var(--grail-gold-soft)', border: '1px solid var(--grail-gold)', borderRadius: 14, padding: 15 }}>
            ...
            <div style={{ marginTop: 11 }}>
              ...
              {poPrec === 'month' && (
                <div style={{ display: 'flex', gap: 10 }}>
                  <select value={poMonth} onChange={e => setPoMonth(e.target.value)} style={poSelectStyle}>
                    <option value="">Month</option>
                    {PO_MONTHS.map((m, i) => <option key={m} value={String(i)}>{m}</option>)}
                  </select>
                  <select value={poYear} onChange={e => setPoYear(e.target.value)} style={poSelectStyle}>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  <div style={{ display: 'flex', gap: 6, flex: 1 }}>
                    {['1', '2', '3', '4'].map(q => {
                      ...
                      return (
                        <button key={q} onClick={() => setPoQuarter(q)} style={{
                  ...
                  <select value={poYear} onChange={e => setPoYear(e.target.value)} style={{ ...poSelectStyle, flex: 'none', width: 96 }}>
                    {PO_YEARS.map(y => <option key={y} value={y}>{y}</option>)}
                  </select>
                </div>
              )}
              ...
              {poPrec === 'tbd' && (
                <div style={{ display: 'flex', gap: 9, alignItems: 'flex-start', padding: '11px 13px', background: 'var(--paper)', border: '1px solid var(--border)', borderRadius: 11, fontSize: 12.5, color: 'var(--ink-soft)', lineHeight: 1.45 }}>
                  ...
                  No date announced yet — it sits under “Date to be announced” on your PO Calendar. Set a window once the maker confirms.
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 11, marginTop: 14 }}>
              <div style={{ flex: 1 }}>
                <Lbl hint="optional">Order date</Lbl>
                <input type="date" value={poOrderDate} onChange={e => setPoOrderDate(e.target.value)}
                  style={{ ...fieldStyle(false), background: 'var(--paper)', fontFamily: 'var(--font-mono)', fontSize: 13 }}/>
              </div>
              <div style={{ flex: 1 }}>
                <Lbl hint="optional">Total price</Lbl>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, height: 46, padding: '0 13px', borderRadius: 12, border: '1px solid var(--border-strong)', background: 'var(--paper)' }}>
            ...
            <div style={{ marginTop: 14 }}><Lbl hint="optional">Deposit paid</Lbl></div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, height: 46, padding: '0 13px', borderRadius: 12, border: '1px solid var(--border-strong)', background: 'var(--paper)' }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 15, color: 'var(--ink-faint)' }}>₹</span>
          ...
          {forSale && (
            <div style={{ padding: '2px 15px 16px', borderTop: '1px solid var(--stamp-red)' }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', marginTop: 13, fontSize: 12, color: 'var(--ink-mute)', lineHeight: 1.5 }}>
                ...
                <span>Listing goes live now. Add clear in-app photos so buyers know exactly what they’re getting.</span>
              </div>
            </div>
          )}
        </div>

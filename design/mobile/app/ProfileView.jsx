    ...
    <Screen header={header} nav={isMe}>
      ...
      <div style={{ padding: '18px 16px 0' }}>
        <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
          <Avatar name={u.name} color={u.color} size={68} verified={u.tier === 'Top Seller' || u.tier === 'Trusted'}/>
          ...
          return (
            <div key={u.handle} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', borderBottom: '1px solid var(--border)' }}>
              <button onClick={() => push({ name: 'profile', user: u.handle })} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}>
                <Avatar name={u.name} color={u.color} size={44} verified={u.tier !== 'Verified'}/>
              </button>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
        ...
        <ValueCard label="Verified items" value={u.verifiedItems} plain accent="var(--verified-teal)"/>
      </div>
